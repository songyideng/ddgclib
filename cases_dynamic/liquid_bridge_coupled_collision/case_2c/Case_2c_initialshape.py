"""Case 2c initial equilibrium shape for the Pitois 2000 Fig. 5 setup.
...
2. Reuse the same volumetric stress operator path used by Case 2c separation.
...
from cases_dynamic.liquid_bridge_coupled_collision.case_2c import Case_2c_pitois2000_separation as sep


OUT_ROOT = Path(__file__).resolve().parent / "out" / "Case_2c_initialshape"
...
# In this Case 2c initial-shape workflow that projector can collapse the
...
USER_INTERACTIVE_ELEV_DEG = sep.USER_INTERACTIVE_ELEV_DEG
USER_INTERACTIVE_AZIM_DEG = sep.USER_INTERACTIVE_AZIM_DEG
...
def initialshape_config() -> sep.VolumetricPitoisConfig:
    return replace(
        ...
        name="Case_2c_initialshape",
        title="Case 2c: equilibrium initial shape",
...
def _outer_profile_samples(state: sep.VolumetricPitoisState) -> tuple[np.ndarray, np.ndarray]:
    sample_s: list[float] = []
    sample_r: list[float] = []
    ...
    return np.asarray(sample_s, dtype=float), np.asarray(sample_r, dtype=float)
...
def _summary_payload(
    ...
    *,
    ...
    live_volume = float(snapshot_volume)
    weight = np.array(
        [0.0, 0.0, -float(state.config.rho_f) * live_volume * float(state.config.gravity_mps2)],
        dtype=float,
    )
    return {
        "config": {
            ...
            "live_volume_ul": float(live_volume * 1.0e9),
...
def _relax_initial_shape(
    ...
    *,
    ...
    sep._update_pressure_projection_scalar(state, dt=working_dt)
    ...
    for step in range(1, max_steps + 1):
        ...
        for attempt in range(max(1, int(USER_STEP_MAX_RETRIES)) + 1):
            ...
            sep._update_duals_and_masses(state)
            sep._update_pressure_scalar(state)
            sep._update_pressure_projection_scalar(state, dt=dt_step)
            symplectic_euler(
                state.HC,
                state.bV_caps,
        ...
        if step % max(1, int(USER_HISTORY_EVERY)) == 0 or step == max_steps:
            history.append(row)
            if (
...
def main() -> None:
    ...
    sep._update_duals_and_masses(state)
    sep._update_pressure_scalar(state)
    sep._update_pressure_projection_scalar(state, dt=float(state.config.dt))
    if USER_UNPIN_CONTACT_LINE:
        sep._update_moving_contact_line(state, dt=None)
        _set_target_msh_volume_metadata(state)
        sep._update_duals_and_masses(state)
    ...
    if USER_RENDER_INITIAL_GUESS:
        sep._render_mesh_snapshot(
            ...
            elev=sep.USER_INTERACTIVE_ELEV_DEG,
            azim=sep.USER_INTERACTIVE_AZIM_DEG,
        )
    ...
    print("=" * 72)
    print("  Case 2c initial equilibrium shape")
    print("=" * 72)
