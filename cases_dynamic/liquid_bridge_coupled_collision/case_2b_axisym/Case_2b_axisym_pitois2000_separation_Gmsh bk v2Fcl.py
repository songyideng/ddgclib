...
USER_ENABLE_INCOMPRESSIBLE_GLOBAL_VOLUME_CONSTRAINT = False
...
USER_MAX_STATE_VELOCITY_MPS = 1.0
USER_INCOMPRESSIBLE_PROJECTION_ALPHA_ABS_MAX = 20.0
USER_INCOMPRESSIBLE_PROJECTION_MAX_SPEED_FRACTION = 0.75
...
# Eq. 3 pressure is evaluated from a local quadratic neck fit.  Keep the hard
# constrained form off by default; v2 uses a weak residual-triggered slope
# penalty instead, so good early fits remain unchanged.
USER_USE_CONSTRAINED_NECK_QUADRATIC = False
USER_ENABLE_NECK_FIT_RESIDUAL_SLOPE_PENALTY = True
USER_NECK_FIT_RESIDUAL_TRIGGER_UM = 1.5
USER_NECK_FIT_RESIDUAL_FULL_PENALTY_UM = 2.5
USER_NECK_FIT_SLOPE_PENALTY = 1.7
# The single waist ring is excluded because its discrete cusp otherwise
# dominates d2r/dz2.
...
class VolumetricPitoisConfig:
    ...
    enable_incompressible_global_volume_constraint: bool = USER_ENABLE_INCOMPRESSIBLE_GLOBAL_VOLUME_CONSTRAINT
    ...
    use_constrained_neck_quadratic: bool = USER_USE_CONSTRAINED_NECK_QUADRATIC
    enable_neck_fit_residual_slope_penalty: bool = USER_ENABLE_NECK_FIT_RESIDUAL_SLOPE_PENALTY
    neck_fit_residual_trigger_um: float = USER_NECK_FIT_RESIDUAL_TRIGGER_UM
    neck_fit_residual_full_penalty_um: float = USER_NECK_FIT_RESIDUAL_FULL_PENALTY_UM
    neck_fit_slope_penalty: float = USER_NECK_FIT_SLOPE_PENALTY
...
class VolumetricPitoisState:
    ...
    incompressible_projection_velocity_scale: float = 1.0
    incompressible_projection_max_speed_before_limiter: float = 0.0
    incompressible_projection_max_speed_after_limiter: float = 0.0
...
def _gmsh_surface_heron_force_map(state: VolumetricPitoisState) -> dict[int, np.ndarray]:
    ...
    if side_tris.size == 0 and bool(getattr(state, "gmsh_compute_mesh", False)):
        side_tris, _bottom_tris, _top_tris = _gmsh_boundary_face_triangle_indices(state)
...
def _neck_fit_penalty_weight(*, residual_rmse_um: float, trigger_um: float, full_penalty_um: float) -> float:
    residual = float(residual_rmse_um)
    trigger = float(trigger_um)
    full = float(full_penalty_um)
    if (not np.isfinite(residual)) or residual <= trigger:
        return 0.0
    if full <= trigger:
        return 1.0
    return float(np.clip((residual - trigger) / (full - trigger), 0.0, 1.0))


def _axisymmetric_neck_quadratic_fit(
    z: np.ndarray,
    r: np.ndarray,
    *,
    side_rings: int,
    constrained: bool = USER_USE_CONSTRAINED_NECK_QUADRATIC,
    enable_residual_slope_penalty: bool = USER_ENABLE_NECK_FIT_RESIDUAL_SLOPE_PENALTY,
    residual_trigger_um: float = USER_NECK_FIT_RESIDUAL_TRIGGER_UM,
    residual_full_penalty_um: float = USER_NECK_FIT_RESIDUAL_FULL_PENALTY_UM,
    slope_penalty: float = USER_NECK_FIT_SLOPE_PENALTY,
    ...
    min_fit_count = 2 if bool(constrained) else 3
    if len(fit_indices) < min_fit_count:
        fit_indices = list(range(lo, hi))
    if len(fit_indices) < min_fit_count:
        return None

    z_ref = float(z[waist_idx])
    idx_arr = np.asarray(fit_indices, dtype=int)
    zz = z[idx_arr] - z_ref
    rr = r[idx_arr]
    residual_rmse_um = 0.0
    penalty_weight = 0.0

    if bool(constrained):
        degree = 2
        design = np.column_stack((zz * zz, np.ones_like(zz)))
        try:
            coeff, _residuals, rank, _singular_values = np.linalg.lstsq(design, rr, rcond=None)
        ...
        b = 0.0
        c = float(coeff[1])
        residual = rr - design @ coeff
        residual_rmse_um = float(np.sqrt(np.mean(np.square(residual))) * 1.0e6)
    else:
        degree = min(2, len(fit_indices) - 1)
        if degree < 1 or np.unique(np.round(zz, 15)).size < degree + 1:
            return None
        try:
            coeff = np.polyfit(zz, rr, degree)
        except (TypeError, ValueError, np.linalg.LinAlgError):
            return None
        fit_rr = np.polyval(coeff, zz)
        residual_rmse_um = float(np.sqrt(np.mean(np.square(rr - fit_rr))) * 1.0e6)
        if degree == 2:
            a, b, c = [float(x) for x in coeff]
            if bool(enable_residual_slope_penalty):
                penalty_weight = _neck_fit_penalty_weight(
                    residual_rmse_um=residual_rmse_um,
                    trigger_um=float(residual_trigger_um),
                    full_penalty_um=float(residual_full_penalty_um),
                )
                effective_penalty = float(slope_penalty) * float(penalty_weight)
                if effective_penalty > 0.0:
                    scale = max(float(np.max(np.abs(zz))), 1.0e-30)
                    q = zz / scale
                    design = np.column_stack((q * q, q, np.ones_like(q)))
                    design_penalized = np.vstack((design, np.array([0.0, effective_penalty, 0.0])))
                    rr_penalized = np.concatenate((rr, np.array([0.0])))
                    try:
                        coeff_scaled, _residuals, rank, _singular_values = np.linalg.lstsq(
                            design_penalized,
                            rr_penalized,
                            rcond=None,
                        )
                        ...
                        return None
                    if int(rank) >= 3:
                        a = float(coeff_scaled[0]) / (scale * scale)
                        b = float(coeff_scaled[1]) / scale
                        c = float(coeff_scaled[2])
        else:
            b, c = [float(x) for x in coeff]
            a = 0.0
    ...
    if (not np.isfinite(r_fit)) or r_fit <= 1.0e-12:
        return None
    dr_dz = float(b)
    d2r_dz2 = float(2.0 * a) if int(degree) == 2 else 0.0
    ...
    return SimpleNamespace(
        kappa=float(kappa),
        radius_m=float(r_fit),
        rz=float(dr_dz),
        rzz_1pm=float(d2r_dz2),
        fit_degree=int(degree),
        neck_ring_index=int(waist_idx),
        fit_indices=[int(idx) for idx in fit_indices],
        residual_rmse_um=float(residual_rmse_um),
        residual_slope_penalty_weight=float(penalty_weight),
    )
...
def _axisymmetric_neck_curvature_from_eq3_fit(
    ...
    *,
    ...
    fit = _axisymmetric_neck_quadratic_fit(
        z,
        r,
        side_rings=side_rings,
        constrained=constrained,
        enable_residual_slope_penalty=enable_residual_slope_penalty,
        residual_trigger_um=residual_trigger_um,
        residual_full_penalty_um=residual_full_penalty_um,
        slope_penalty=slope_penalty,
    )
    if fit is None:
        return None
    return float(fit.kappa)
...
def _update_pressure_scalar(state: VolumetricPitoisState) -> None:
    ...
    kappa_fit = _axisymmetric_neck_curvature_from_eq3_fit(
        ...
        constrained=bool(
            getattr(state.config, "use_constrained_neck_quadratic", USER_USE_CONSTRAINED_NECK_QUADRATIC)
        ),
        enable_residual_slope_penalty=bool(
            getattr(
                state.config,
                "enable_neck_fit_residual_slope_penalty",
                USER_ENABLE_NECK_FIT_RESIDUAL_SLOPE_PENALTY,
            )
        ),
        residual_trigger_um=float(
            getattr(state.config, "neck_fit_residual_trigger_um", USER_NECK_FIT_RESIDUAL_TRIGGER_UM)
        ),
        residual_full_penalty_um=float(
            getattr(state.config, "neck_fit_residual_full_penalty_um", USER_NECK_FIT_RESIDUAL_FULL_PENALTY_UM)
        ),
        slope_penalty=float(getattr(state.config, "neck_fit_slope_penalty", USER_NECK_FIT_SLOPE_PENALTY)),
    )
...
def _state_finite_summary(state: VolumetricPitoisState) -> tuple[bool, str]:
    ...
    max_state_velocity = max(float(USER_MAX_STATE_VELOCITY_MPS), 0.0)
    if not np.isfinite(max_vel) or max_vel > max_state_velocity:
...
def _Fcl(v, *, state: VolumetricPitoisState) -> np.ndarray:
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        contact_ids = {id(node) for node in state.bottom_contact_ring + state.top_contact_ring}
        if id(v) in contact_ids:
...
def _gmsh_axisymmetrize_velocity_array(
    state: VolumetricPitoisState,
    vertices: list,
    velocity_array: np.ndarray,
) -> np.ndarray:
    velocities = np.asarray(velocity_array, dtype=float).reshape((len(vertices), 3)).copy()
    if (not bool(USER_ENFORCE_FULL_AXISYMMETRY)) or (not bool(getattr(state, "gmsh_compute_mesh", False))):
        return velocities
    specs = list(getattr(state, "gmsh_axisym_ring_specs", []))
    if not specs:
    ...
    for spec in specs:
        ...
        units = _gmsh_axisym_spec_radial_units(spec, e1, e2)
        pairs = [
            (vertex, index_by_id[id(vertex)], e_r)
            for vertex, e_r in zip(spec_vertices, units)
            if id(vertex) in index_by_id
        ]
        if not pairs:
            continue
        if len(pairs) == 1 or float(getattr(spec, "r0", 0.0)) <= 1.0e-30:
            uz = float(np.mean([float(np.dot(velocities[idx], axis)) for _vertex, idx, _e_r in pairs]))
            for _vertex, idx, _e_r in pairs:
                velocities[idx] = uz * axis
                covered.add(idx)
            continue

        ur = float(np.mean([float(np.dot(velocities[idx], e_r)) for _vertex, idx, e_r in pairs]))
        uz = float(np.mean([float(np.dot(velocities[idx], axis)) for _vertex, idx, _e_r in pairs]))
        cap_id = _gmsh_axisym_uniform_cap_id(spec_vertices)
        is_free_interface = any(bool(getattr(vertex, "is_interface", False)) for vertex in spec_vertices)
        if (
            bool(USER_CLAMP_INTERFACE_RADIUS_TO_CONTACT)
            and cap_id is None
            and is_free_interface
        ):
            coords = np.asarray([np.asarray(vertex.x_a[:3], dtype=float) for vertex in spec_vertices], dtype=float)
            rel = coords - mid[None, :]
            axial = np.dot(rel, axis)
            radial = rel - np.outer(axial, axis)
            r_mean = float(np.mean(np.linalg.norm(radial, axis=1)))
        ...
        if (
            ...
            if (center_axial <= lo + span_margin and uz < 0.0) or (center_axial >= hi - span_margin and uz > 0.0):
                uz = 0.0
        for _vertex, idx, e_r in pairs:
            velocities[idx] = ur * e_r + uz * axis
            covered.add(idx)

    for idx, vertex in enumerate(vertices):
        if idx in covered:
            continue
        velocities[idx] = _remove_swirl_component(
            velocities[idx],
...
def _projection_velocity_limit_mps() -> float:
    max_state_velocity = float(USER_MAX_STATE_VELOCITY_MPS)
    speed_fraction = float(USER_INCOMPRESSIBLE_PROJECTION_MAX_SPEED_FRACTION)
    if (
        (not np.isfinite(max_state_velocity))
        or max_state_velocity <= 0.0
        or (not np.isfinite(speed_fraction))
        or speed_fraction <= 0.0
    ):
        return float("inf")
    return float(max_state_velocity * min(speed_fraction, 1.0))


def _limit_velocity_array_norms(
    velocities: np.ndarray,
    *,
    fixed_mask: np.ndarray,
    max_speed: float,
) -> tuple[np.ndarray, float, float, float]:
    limited = np.asarray(velocities, dtype=float).copy()
    if limited.size == 0:
        return limited, 1.0, 0.0, 0.0
    if (not np.isfinite(max_speed)) or max_speed <= 0.0:
        ...
        return limited, 1.0, max_norm, max_norm

    mask = ~np.asarray(fixed_mask, dtype=bool)
    norms = np.linalg.norm(limited, axis=1)
    max_before = float(np.max(norms[mask])) if np.any(mask) else 0.0
    scale = 1.0
    too_fast = mask & np.isfinite(norms) & (norms > float(max_speed))
    if np.any(too_fast):
        factors = float(max_speed) / np.maximum(norms[too_fast], 1.0e-30)
        limited[too_fast] *= factors[:, None]
        scale = float(np.min(factors))
    norms_after = np.linalg.norm(limited, axis=1)
    max_after = float(np.max(norms_after[mask])) if np.any(mask) else 0.0
    return limited, scale, max_before, max_after


def _limit_gmsh_free_vertex_speeds(
    state: VolumetricPitoisState,
    *,
    ...
    if not bool(getattr(state, "gmsh_compute_mesh", False)):
        return
    vertex_list = list(vertices) if vertices is not None else list(getattr(state, "volume_export_vertices", []))
    if not vertex_list:
        return
    fixed = set(state.bV_caps)
    velocities = np.asarray([np.asarray(v.u[:3], dtype=float) for v in vertex_list], dtype=float)
    fixed_mask = np.asarray([v in fixed for v in vertex_list], dtype=bool)
    limited, scale, max_before, max_after = _limit_velocity_array_norms(
        velocities,
        fixed_mask=fixed_mask,
        max_speed=_projection_velocity_limit_mps(),
    )
    for idx, vertex in enumerate(vertex_list):
        if fixed_mask[idx]:
            continue
        vertex.u = np.nan_to_num(limited[idx], nan=0.0, posinf=0.0, neginf=0.0)
    state.incompressible_projection_velocity_scale = float(scale)
    state.incompressible_projection_max_speed_before_limiter = float(max_before)
    state.incompressible_projection_max_speed_after_limiter = float(max_after)
...
def _apply_gmsh_incompressible_projection(state: VolumetricPitoisState, *, dt: float) -> None:
    ...
    target_volume = float(getattr(state, "target_snapshot_volume_m3", 0.0))
    if target_volume > 0.0 and bool(
        getattr(
            state.config,
            "enable_incompressible_global_volume_constraint",
            USER_ENABLE_INCOMPRESSIBLE_GLOBAL_VOLUME_CONSTRAINT,
        )
    ):
    ...
    alpha = 1.0
    alpha_abs_max = max(float(USER_INCOMPRESSIBLE_PROJECTION_ALPHA_ABS_MAX), 1.0)
    mid, axis, e1, e2 = _gmsh_axisym_basis(state)
    ...
    def predicted_step_volume(alpha_value: float) -> float:
        ...
        trial_velocity = _gmsh_axisymmetrize_velocity_array(state, vertices, trial_velocity)
    ...
    if target_volume > 0.0:
        ...
        def add_candidate(alpha_value: float) -> tuple[float, float] | None:
            if not np.isfinite(alpha_value):
                return None
            alpha_value = float(np.clip(alpha_value, -alpha_abs_max, alpha_abs_max))
        ...
        if needs_bracket:
            ...
            for span in (50.0, 100.0, 200.0, 500.0, 1000.0, 2000.0):
                span = min(float(span), alpha_abs_max)
    ...
    projected_velocity = _gmsh_axisymmetrize_velocity_array(state, vertices, projected_velocity)
    fixed_mask = np.asarray([v in fixed for v in vertices], dtype=bool)
    projected_velocity, velocity_scale, max_speed_before, max_speed_after = _limit_velocity_array_norms(
        projected_velocity,
        fixed_mask=fixed_mask,
        max_speed=_projection_velocity_limit_mps(),
    )
    state.incompressible_projection_velocity_scale = float(velocity_scale)
    state.incompressible_projection_max_speed_before_limiter = float(max_speed_before)
    state.incompressible_projection_max_speed_after_limiter = float(max_speed_after)
    for idx, v in enumerate(vertices):
        if v in fixed:
            continue
    ...
    _gmsh_axisymmetrize_velocity_field(state)
    _set_cap_velocities(state)
    _limit_gmsh_free_vertex_speeds(state, vertices=vertices)
    _set_cap_velocities(state)
...
def _advance_gmsh_incompressible_substep(state: VolumetricPitoisState, *, dt: float) -> bool:
    ...
    _set_cap_velocities(state)
    _limit_gmsh_free_vertex_speeds(state, vertices=list(getattr(state, "volume_export_vertices", [])))
    _set_cap_velocities(state)
...
def _axisymmetric_neck_fit_diagnostics(state: VolumetricPitoisState) -> SimpleNamespace:
    p0_pa = float(getattr(state, "pressure_scalar", 0.0))
    gamma = max(float(getattr(state.config, "gamma", 0.0)), 1.0e-30)
    default_H = p0_pa / gamma
    nonempty_rings = [(idx, ring) for idx, ring in enumerate(state.outer_rings) if ring]
    neck_ring_index = -1
    neck_vertex_count = 0
    neck_radius_m = 0.0
    if nonempty_rings:
        neck_ring_index, neck_ring = min(nonempty_rings, key=lambda item: float(_cap_radius(item[1])))
        neck_vertex_count = len(neck_ring)
        neck_radius_m = float(_cap_radius(neck_ring))

    z, r, kappa = _axisymmetric_profile(state)
    finite_kappa = np.asarray(kappa, dtype=float)
    finite_kappa = finite_kappa[np.isfinite(finite_kappa)]
    H_avg = float(np.mean(finite_kappa)) if finite_kappa.size else float(default_H)
    vertex_count = int(sum(len(ring) for ring in state.outer_rings if ring))
    result = {
        "p0_pa": p0_pa,
        "delta_p_pa": abs(p0_pa),
        "H_1pm": float(default_H),
        "H_avg_1pm": H_avg,
        "H_avg_vertex_count": vertex_count,
        "radius_m": neck_radius_m,
        "rz": 0.0,
        "rzz_1pm": 0.0,
        "vertex_count": int(neck_vertex_count),
    ...
    r = np.asarray(r, dtype=float)[valid]
    if z.size < 3:
        return SimpleNamespace(**result)

    side = max(1, int(getattr(state.config, "pressure_neck_fit_side_rings", USER_PRESSURE_NECK_FIT_SIDE_RINGS)))
    constrained = bool(
        getattr(state.config, "use_constrained_neck_quadratic", USER_USE_CONSTRAINED_NECK_QUADRATIC)
    )
    fit = _axisymmetric_neck_quadratic_fit(
    ...
    if np.isfinite(float(fit.radius_m)) and float(fit.radius_m) > 0.0:
        result["radius_m"] = float(fit.radius_m)
    result["rz"] = float(fit.rz) if np.isfinite(float(fit.rz)) else 0.0
    result["rzz_1pm"] = float(fit.rzz_1pm) if np.isfinite(float(fit.rzz_1pm)) else 0.0
    result["fit_half_window_rings"] = int(side)
    result["fit_degree"] = int(fit.fit_degree)
    return SimpleNamespace(**result)


def _step_record(state: VolumetricPitoisState, *, step: int, t: float) -> dict[str, float | int]:
    sphere_forces = _sphere_total_forces(state)
    top_force = sphere_forces["top_cap"]
    bottom_force = sphere_forces["bottom_cap"]
    ...
    neck_fit = _axisymmetric_neck_fit_diagnostics(state)
    ...
    top_cl_avg_dxyz = top_cl_avg_u * float(state.last_step_dt)
    top_cap_axial = float(sphere_forces["top_cap"][2])
    top_line_axial = float(sphere_forces["top_line"][2])
    top_total_axial = float(top_cap_axial)
    top_cl_radius = float(_cap_radius(state.outer_rings[-1])) if state.outer_rings else 0.0
    ...
    return {
        ...
        "p0_neck_fit_pa": float(neck_fit.p0_pa),
        "p0_neck_fit_delta_p_pa": float(neck_fit.delta_p_pa),
        "p0_neck_fit_H_1pm": float(neck_fit.H_1pm),
        "diagnostic_lv_H_avg_1pm": float(neck_fit.H_avg_1pm),
        "diagnostic_lv_H_avg_vertex_count": int(neck_fit.H_avg_vertex_count),
        "p0_neck_fit_radius_mm": 1.0e3 * float(neck_fit.radius_m),
        "p0_neck_fit_rz": float(neck_fit.rz),
        "p0_neck_fit_rzz_1pm": float(neck_fit.rzz_1pm),
        "p0_neck_fit_vertex_count": int(neck_fit.vertex_count),
        "p0_neck_fit_half_window_rings": int(neck_fit.fit_half_window_rings),
        "p0_neck_fit_degree": int(neck_fit.fit_degree),
        "p0_neck_ring_index": int(neck_fit.neck_ring_index),
        ...
        "top_cl_avg_Finterface_total_mN": float(top_cl_avg_line_mn),
        ...
        "top_sphere_Finterface_total_mN": float(1.0e3 * top_total_axial),
        "top_sphere_Ftot_mN": float(1.0e3 * top_total_axial),
        "top_sphere_force_mN": float(1.0e3 * top_cap_axial),
        "top_sphere_force_n": float(top_cap_axial),
        ...
        "incompressible_divergence_before_l2": float(state.incompressible_divergence_before_l2),
        "incompressible_divergence_after_l2": float(state.incompressible_divergence_after_l2),
        "incompressible_projection_alpha": float(getattr(state, "incompressible_projection_alpha", 1.0)),
        "incompressible_projection_velocity_scale": float(state.incompressible_projection_velocity_scale),
        "incompressible_projection_max_speed_before_limiter": float(
            state.incompressible_projection_max_speed_before_limiter
        ),
        "incompressible_projection_max_speed_after_limiter": float(
            state.incompressible_projection_max_speed_after_limiter
        ),
...
OUTPUT_CSV_FIELDS = [
    ...
    "top_sphere_force_mN",
    "max_free_speed_mps",
    "dt_limit_contact_line_s",
    "dt_limit_capillary_s",
    "dt_limit_mesh_s",
    "dt_limiter",
    "bottom_contact_line_speed_mps",
    "top_contact_line_speed_mps",
    "div_u_before_l2_1ps",
    "div_u_after_l2_1ps",
    "projection_alpha",
    "projection_velocity_scale",
    "projection_max_speed_before_limiter_mps",
    "projection_max_speed_after_limiter_mps",
]
...
def _output_csv_row(row: dict) -> dict[str, float | int]:
    ...
    return {
        ...
        "max_free_speed_mps": float(row.get("max_free_speed", 0.0)),
        "dt_limit_contact_line_s": float(row.get("dt_limit_contact_line", 0.0)),
        "dt_limit_capillary_s": float(row.get("dt_limit_capillary", 0.0)),
        "dt_limit_mesh_s": float(row.get("dt_limit_mesh", 0.0)),
        "dt_limiter": str(row.get("dt_limiter", "")),
        "bottom_contact_line_speed_mps": float(row.get("bottom_contact_line_speed", 0.0)),
        "top_contact_line_speed_mps": float(row.get("top_contact_line_speed", 0.0)),
        "div_u_before_l2_1ps": float(row.get("incompressible_divergence_before_l2", 0.0)),
        "div_u_after_l2_1ps": float(row.get("incompressible_divergence_after_l2", 0.0)),
        "projection_alpha": float(row.get("incompressible_projection_alpha", 1.0)),
        "projection_velocity_scale": float(row.get("incompressible_projection_velocity_scale", 1.0)),
        "projection_max_speed_before_limiter_mps": float(
            row.get("incompressible_projection_max_speed_before_limiter", 0.0)
        ),
        "projection_max_speed_after_limiter_mps": float(
            row.get("incompressible_projection_max_speed_after_limiter", 0.0)
        ),
    }
...
def _history_arrays(history: list[dict]) -> dict[str, np.ndarray]:
    ...
    force_mn = np.array(
        [
            float(row.get("Ftopspherecap_p0_mN", 1.0e3 * float(row["fixed_force_axial"])))
            for row in history
        ],
        dtype=float,
    )
...
class _PitoisFig5RefreshMonitor:
    def __init__(
        self,
        *,
        out_dir: Path,
        xlim: tuple[float, float] = (0.01, 0.30),
        ylim: tuple[float, float] = (0.02, 2.0),
        title: str = "Pitois 2000 Fig. 5: dynamic experiment vs Case_2b_axisym",
    ) -> None:
        self.out_path = out_dir / "pitois2000_volumetric_separation_digitized_compare.png"
        self.out_path.parent.mkdir(parents=True, exist_ok=True)
        self.xlim = (float(xlim[0]), float(xlim[1]))
        self.ylim = (float(ylim[0]), float(ylim[1]))
        self.exp_x, self.exp_y = _pitois_fig5_dynamic_digitized()
        self.sim_x_all: list[float] = []
        self.sim_y_all: list[float] = []
        self.sim_x_compare: list[float] = []
        self.sim_y_compare: list[float] = []
        self.all_visible_min: float | None = None
        self.all_visible_max: float | None = None
        self.compare_visible_min: float | None = None
        self.compare_visible_max: float | None = None
        self.exp_visible_min: float | None = None
        self.exp_visible_max: float | None = None
        ...
        self.ax.scatter(
            ...
            s=34,
            color="#111111",
            label="Pitois 2000 dynamic exp (black dots)",
        )
        (self.sim_line,) = self.ax.plot(
            [],
            [],
            color="#d62828",
            linewidth=2.0,
            label="Case_2b_axisym simulation",
        )
        self.ax.set_xscale("log")
        self.ax.set_yscale("log")
        self.ax.set_xlim(*self.xlim)
        self.ax.set_xlabel(r"$D/R$")
        self.ax.set_ylabel(r"$|F|$ [mN]")
        self.ax.set_title(title)
        self.ax.grid(alpha=0.28, which="both")
        self.ax.legend()
        decimal_log_formatter = FuncFormatter(lambda value, _pos: f"{float(value):g}" if value > 0.0 else "")
        self.ax.xaxis.set_major_formatter(decimal_log_formatter)
        self.ax.yaxis.set_major_formatter(decimal_log_formatter)
        self.ax.xaxis.set_minor_formatter(NullFormatter())
        self.ax.yaxis.set_minor_formatter(NullFormatter())
        self._initialize_experiment_ylim()
        self._update_ylim(use_compare=False)
        self.fig.tight_layout()

    def _initialize_experiment_ylim(self) -> None:
        x_vals = np.asarray(self.exp_x, dtype=float)
        y_vals = np.asarray(self.exp_y, dtype=float)
        mask = (
            np.isfinite(x_vals)
            & np.isfinite(y_vals)
            & (x_vals >= self.xlim[0])
        ...
        if np.any(mask):
            visible = y_vals[mask]
            self.exp_visible_min = float(np.min(visible))
            self.exp_visible_max = float(np.max(visible))
    ...
    def _update_min_max(
        ...
        return min(float(current_min), float(value)), max(float(current_max), float(value))

    def _update_ylim(self, *, use_compare: bool) -> None:
        visible_min = self.exp_visible_min
        visible_max = self.exp_visible_max
        sim_min = self.compare_visible_min if use_compare else self.all_visible_min
        sim_max = self.compare_visible_max if use_compare else self.all_visible_max
        if sim_min is not None and sim_max is not None:
            if visible_min is None or visible_max is None:
                visible_min = float(sim_min)
                visible_max = float(sim_max)
            else:
                visible_min = min(float(visible_min), float(sim_min))
                visible_max = max(float(visible_max), float(sim_max))

        y_low, y_high = self.ylim
        if visible_min is not None and visible_max is not None:
            y_low = min(float(y_low), 0.85 * float(visible_min))
            y_high = max(float(y_high), 1.15 * float(visible_max))
        self.ax.set_ylim(float(y_low), float(y_high))

    def append_and_save(self, row: dict) -> Path:
        step = int(row.get("step", 0))
        x_val = float(row.get("d_over_r", 0.0))
        if "Ftopspherecap_p0_mN" in row:
            y_val = abs(float(row.get("Ftopspherecap_p0_mN", 0.0)))
        else:
            force_n = float(row.get("fixed_force_axial", row.get("top_sphere_force_n", 0.0)))
            y_val = abs(1.0e3 * force_n)
        self.sim_x_all.append(x_val)
        self.sim_y_all.append(y_val)

        is_visible = (
            np.isfinite(x_val)
            and np.isfinite(y_val)
            and x_val >= self.xlim[0]
            and x_val <= self.xlim[1]
            and y_val > 0.0
        )
        if is_visible:
            self.all_visible_min, self.all_visible_max = self._update_min_max(
                y_val,
                self.all_visible_min,
                self.all_visible_max,
            )

        keep_compare = step > int(USER_FIG5_COMPARE_SKIP_INITIAL_STEPS)
        if keep_compare:
            self.sim_x_compare.append(x_val)
            self.sim_y_compare.append(y_val)
            if is_visible:
                self.compare_visible_min, self.compare_visible_max = self._update_min_max(
                    y_val,
                    self.compare_visible_min,
                    self.compare_visible_max,
                )

        use_compare = bool(self.sim_x_compare)
        self.sim_line.set_data(
            self.sim_x_compare if use_compare else self.sim_x_all,
            self.sim_y_compare if use_compare else self.sim_y_all,
        )
        self._update_ylim(use_compare=use_compare)
        self.fig.savefig(self.out_path, dpi=200, bbox_inches="tight")
...
def run_motion_case(
    ...
    *,
    ...
    if save_results:
        out_dir.mkdir(parents=True, exist_ok=True)
        output_csv_file = open(out_dir / "output.csv", "w", newline="", encoding="utf-8")
        output_csv_writer = csv.DictWriter(output_csv_file, fieldnames=OUTPUT_CSV_FIELDS)
    ...
    pitois_fig5_monitor = _PitoisFig5RefreshMonitor(out_dir=out_dir) if save_fig else None
    ...
    for step in range(config.n_steps):
        ...
        if output_csv_writer is not None and output_csv_file is not None:
            output_csv_writer.writerow(_output_csv_row(record_row))
            output_csv_file.flush()
        if pitois_fig5_monitor is not None:
            pitois_fig5_monitor.append_and_save(record_row)
        if verbose:
            snapshot_volume_ul = float(record_row.get("water_filled_mesh_volume_uL", 0.0))
            rel_snapshot_volume_error = float(record_row.get("water_vol_error_rel", 0.0))
            print(
