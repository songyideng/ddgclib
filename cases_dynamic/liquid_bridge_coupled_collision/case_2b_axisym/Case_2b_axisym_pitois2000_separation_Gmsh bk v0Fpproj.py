...
# Gmsh path: use the scalar Fp,proj volume-force controller from v0, without
# the tetrahedral velocity-pressure continuity projection.
...
# Eq. 3 pressure is evaluated from a local quadratic neck fit.  The constrained
# form enforces r_z = 0 at the fitted neck, which damps late-stage slope noise.
...
def _axisymmetric_neck_fit_diagnostics(state: VolumetricPitoisState) -> SimpleNamespace:
    ...
    min_fit_count = 2 if constrained else 3
    if len(fit_indices) < min_fit_count:
        fit_indices = list(range(lo, hi))
    degree = 2 if constrained else min(2, len(fit_indices) - 1)
    if degree < 1:
        return SimpleNamespace(**result)
    ...
    if constrained:
        ...
        rz = 0.0
        rzz = 2.0 * a
        radius_fit = float(coeff[1])
    else:
        if np.unique(np.round(zz, 15)).size < degree + 1:
            return SimpleNamespace(**result)
        try:
        ...
        if degree == 2:
            ...
            rzz = 0.0
            radius_fit = c
    if np.isfinite(radius_fit) and radius_fit > 0.0:
        result["radius_m"] = float(radius_fit)
    result["rz"] = float(rz) if np.isfinite(rz) else 0.0
    result["rzz_1pm"] = float(rzz) if np.isfinite(rzz) else 0.0
    result["fit_half_window_rings"] = int(side)
    result["fit_degree"] = int(degree)
    return SimpleNamespace(**result)
