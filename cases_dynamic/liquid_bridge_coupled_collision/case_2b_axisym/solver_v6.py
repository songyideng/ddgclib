"""Computation-only solver for Case 2b Gmsh Pitois-2000 separation v6.
...
``Case_2b_axisym_pitois2000_separation_Gmsh v6.py``.
"""
...
except Exception:  # pragma: no cover - optional SciPy path checked at runtime
    coo_matrix = None
    diags = None
    spsolve = None
...
V6_CASE_DIR = Path(__file__).resolve().parent / "Case_2b_axisym_pitois2000_separation_Gmsh v6"
V6_GENERATED_INITIAL_MSH_PATH = V6_CASE_DIR / "initial_bridge_v6.msh"
# Backward-compatible aliases for the unused v5 mesh-generation helpers kept
# below.  v6 never calls them by default; _initial_msh_path_for_run returns the
# trusted hard-coded mesh.
V5_CASE_DIR = V6_CASE_DIR
V5_GENERATED_INITIAL_MSH_PATH = V6_GENERATED_INITIAL_MSH_PATH
...
USER_ENABLE_POST_STEP_STOKES_DIAGNOSTIC_SOLVE = False
...
# v6 deliberately keeps the trusted mesh_iter0012.msh initial geometry and
# computes p0 with the same neck-side-ring Eq. 3 fit used by bk v0.
USER_P0_MERIDIAN_FIT_HALF_WINDOW_RINGS = 2
...
USER_V5_GENERATE_INITIAL_MSH = False
USER_V5_REGENERATE_INITIAL_MSH = False
...
class VolumetricPitoisState:
    ...
    incompressible_projection_alpha: float = 1.0
    incompressible_projection_volume_lambda: float = 0.0
    incompressible_projection_uses_global_volume_constraint: bool = False
...
def separation_config() -> VolumetricPitoisConfig:
    return VolumetricPitoisConfig(
        name="Case_2b_axisym_pitois2000_separation_Gmsh v6",
...
def _move_vertices_batch(vertices, targets, HC, bV) -> None:
    ...
    current = np.asarray([np.asarray(v.x_a[:3], dtype=float) for v in vertices], dtype=float)
    target_array = np.asarray(targets, dtype=float)
    move_mask = np.linalg.norm(current - target_array, axis=1) > 1.0e-15
    if not bool(np.any(move_mask)):
        return
    if not bool(np.all(move_mask)):
        vertices = [vertex for vertex, keep in zip(vertices, move_mask) if bool(keep)]
        targets = [target for target, keep in zip(targets, move_mask) if bool(keep)]
    if len(vertices) == 1:
...
def _base_update_duals_and_masses(state: VolumetricPitoisState) -> None:
    ...
    if vertices and tets.size:
        ...
        pc = tet_points[:, 2, :]
        pd = tet_points[:, 3, :]
        volumes = np.abs(np.einsum("ij,ij->i", pa - pd, np.cross(pb - pd, pc - pd))) / 6.0
        shares = 0.25 * volumes
        lumped = np.zeros(len(vertices), dtype=float)
        np.add.at(lumped, tets[:, 0], shares)
        np.add.at(lumped, tets[:, 1], shares)
        np.add.at(lumped, tets[:, 2], shares)
        np.add.at(lumped, tets[:, 3], shares)
...
def _static_p0_meridian_fit_diagnostics(state: VolumetricPitoisState) -> SimpleNamespace:
    ...
    fit_indices = [idx for idx in range(start, stop) if idx != neck_ring_index]
    if len(fit_indices) < 3:
        fit_indices = list(range(start, stop))
    ...
    if fit_count < 3 or fit_degree < 2:
        ...
        fit_idx_arr = np.asarray(fit_indices, dtype=int)
        x = axial_arr[fit_idx_arr] - float(axial_arr[neck_ring_index])
        y = radii_arr[fit_idx_arr]
        coeff = np.polyfit(x, y, deg=fit_degree)
        poly = np.poly1d(coeff)
        dpoly = np.polyder(poly, 1)
        ddpoly = np.polyder(poly, 2)
        fit_radius_m = max(float(poly(0.0)), 1.0e-30)
        rz = float(dpoly(0.0))
        rzz_1pm = float(ddpoly(0.0))
        slope_factor = 1.0 + rz * rz
        # Same Eq. 3 sign convention as bk v0:
        #   kappa = -r_zz / (1 + r_z^2)^(3/2)
        #           + 1 / (r sqrt(1 + r_z^2))
        # and p0 = gamma * kappa.  The exact neck ring is excluded from the fit
        # to avoid letting a single discrete cusp dominate r_zz.
        H_1pm = (
            -rzz_1pm / (slope_factor ** 1.5)
            + 1.0 / (fit_radius_m * math.sqrt(slope_factor))
        )
        fit_x = x
        ...
        fit_weights = np.asarray(ring_count_arr[fit_idx_arr], dtype=float)
        if fit_H.size and np.sum(fit_weights) > 0.0:
            H_avg_1pm = float(np.average(fit_H, weights=fit_weights))
        else:
            H_avg_1pm = float(H_1pm)
        p0_pa = float(state.config.gamma) * float(H_1pm)
        delta_p_pa = abs(float(p0_pa))
    ...
    cached = SimpleNamespace(
        ...
        vertex_count=int(np.sum(ring_count_arr[np.asarray(fit_indices, dtype=int)])) if fit_count > 0 else 0,
...
def _stokes_force_load_map(state: VolumetricPitoisState) -> dict[int, np.ndarray]:
    """Actual nodal loads that drive the Stokes velocity solve.

    v6 must let the neck geometry evolve, because the compared force is
    F_top = p0(t) * pi * r_neck(t)^2.  Subtracting the t=0 reference load makes
    the initial capillary state invisible to the motion solve and effectively
    freezes p0 and r_neck, so the solver uses the current load directly.
    """
    return {
...
def _solve_stokes_continuity_flow(state: VolumetricPitoisState, *, dt: float) -> bool:
    ...
    fixed_velocity_rows: set[int] = set()
    for vertex_id in boundary_velocity_map:
        idx = id_to_idx.get(vertex_id)
        if idx is None:
            continue
        fixed_velocity_rows.update((3 * idx + 0, 3 * idx + 1, 3 * idx + 2))
        ...
        for a, ia in enumerate(ids):
            for b, ib in enumerate(ids):
                ...
                for i_dim in range(3):
                    ...
                    p_col = n_velocity + ib
                    value = -(volume / 4.0) * float(grad_phi[a, i_dim])
                    if abs(value) > 0.0:
                        rows.append(row)
                        cols.append(p_col)
                        data.append(float(value))
                    ...
                    if p_row == gauge_row:
                        continue
                    u_col = 3 * ib + i_dim
                    value = (volume / 4.0) * float(grad_phi[b, i_dim])
                    if abs(value) > 0.0:
                        rows.append(p_row)
                        cols.append(u_col)
                        data.append(float(value))
    ...
    for vertex_id, force_load in force_load_map.items():
        ...
        for i_dim in range(3):
            ...
            if row in fixed_velocity_rows:
                continue
            rhs[row] -= float(force_load[i_dim])
    ...
    rows.append(gauge_row)
    cols.append(gauge_row)
    data.append(1.0)
...
def _apply_gmsh_incompressible_projection(state: VolumetricPitoisState, *, dt: float) -> None:
    ...
    if target_volume > 0.0:
        ...
        def add_candidate(alpha_value: float) -> tuple[float, float] | None:
            ...
            alpha_key = round(alpha_value, 12)
            if alpha_key in seen_alpha:
                return None
...
def _solver_top_sphere_force_components_n(state: VolumetricPitoisState) -> SimpleNamespace:
    _bottom_center, _top_center, axis = _particle_centers_physical(state)
    p0_diag = _static_p0_meridian_fit_diagnostics(state)
    top_sphere_Fp0_n = float(p0_diag.p0_pa) * math.pi * float(p0_diag.radius_m) ** 2

    # Keep the raw Cauchy-Stokes and Pitois lubrication values as explicit
    # diagnostics only.  v6 restores the trusted bk-v0 comparison definition:
    # the measured Fig. 5 baseline force is the neck/gorge pressure force.
    # Do not silently replace it by the generated-geometry sphere-cap stress
    # path or by an unvalidated lubrication correction.
    ...
    top_sphere_Ftau_diag_n = float(np.dot(top_cap_stress.Ftau, axis))
    ...
    top_sphere_Fp_flow_n = 0.0
    top_sphere_Ftau_n = 0.0
    top_sphere_Fs_n = 0.0
    ...
    # v6 comparison force:
    #     F_top = p0_neck * pi * r_neck^2
    # This intentionally matches the trusted bk-v0 t=0 force path.
...
def advance_one_step(state: VolumetricPitoisState, *, step_index: int | None = None) -> float:
    ...
    for _substep in range(substeps):
        ...
        _update_duals_and_masses(state)
        _update_pressure_scalar(state)
        if bool(USER_ENABLE_POST_STEP_STOKES_DIAGNOSTIC_SOLVE):
