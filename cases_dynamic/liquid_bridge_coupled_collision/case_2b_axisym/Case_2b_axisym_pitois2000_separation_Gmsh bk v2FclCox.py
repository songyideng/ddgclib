...
USER_ENABLE_INCOMPRESSIBLE_GLOBAL_VOLUME_CONSTRAINT = True
USER_ENABLE_VOLUME_PROJECTION = False
USER_ENABLE_PRESSURE_TRIAL_REFINEMENT = False
...
def _cox_contact_line_speed(state: VolumetricPitoisState, *, which: str) -> float:
    if which == "top":
        ring = state.top_contact_ring
        saved_speed = float(getattr(state, "last_top_contact_line_speed", 0.0))
    else:
        ring = state.bottom_contact_ring
        saved_speed = float(getattr(state, "last_bottom_contact_line_speed", 0.0))
    if not ring:
        return 0.0
    bottom_center, top_center, axis = _particle_centers_physical(state)
    sphere_center = top_center if which == "top" else bottom_center
    instantaneous_speed = abs(
    ...
    return max(abs(saved_speed), float(instantaneous_speed))


def _cox_contact_line_sphere_force(state: VolumetricPitoisState, *, which: str) -> np.ndarray:
    ...
    radius = float(_cap_radius(ring))
    if radius <= 1.0e-30:
        return np.zeros(3, dtype=float)
    speed = _cox_contact_line_speed(state, which=which)
    theta = _dynamic_contact_angle_from_speed(state, slide_speed=speed)
    magnitude = 2.0 * np.pi * radius * float(state.config.gamma) * max(float(np.cos(theta)), 0.0)
    sign = 1.0 if which == "top" else -1.0
    return sign * magnitude * axis


def _cox_contact_line_liquid_force_map(state: VolumetricPitoisState) -> dict[int, np.ndarray]:
    cached = getattr(state, "_cox_contact_line_force_cache", None)
    if cached is not None:
        return cached
    force_map: dict[int, np.ndarray] = {}
    for which, ring in (("bottom", state.bottom_contact_ring), ("top", state.top_contact_ring)):
        if not ring:
            continue
        liquid_total = -_cox_contact_line_sphere_force(state, which=which)
        seg_map = _ring_segment_length_map(ring)
        total_length = float(sum(max(float(seg_map.get(id(v), 0.0)), 0.0) for v in ring))
        for v in ring:
            if total_length > 1.0e-30:
                weight = max(float(seg_map.get(id(v), 0.0)), 0.0) / total_length
            else:
                weight = 1.0 / max(len(ring), 1)
            force_map[id(v)] = force_map.get(id(v), np.zeros(3, dtype=float)) + liquid_total * weight
    state._cox_contact_line_force_cache = force_map
    return force_map
    ...
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        ...
        if id(v) in contact_ids:
            if bool(getattr(state.config, "use_cox_contact_line_force", USER_USE_COX_CONTACT_LINE_FORCE)):
                force = _cox_contact_line_liquid_force_map(state).get(id(v))
            else:
...
def _apply_gmsh_incompressible_projection(state: VolumetricPitoisState, *, dt: float) -> None:
    ...
    if target_volume > 0.0 and bool(USER_ENABLE_PRESSURE_TRIAL_REFINEMENT):
...
def _ring_surface_tension_force(ring: list, *, gamma: float, state: VolumetricPitoisState | None = None) -> np.ndarray:
    ...
    if state is not None and bool(getattr(state, "gmsh_compute_mesh", False)):
        if bool(getattr(state.config, "use_cox_contact_line_force", USER_USE_COX_CONTACT_LINE_FORCE)):
            force_map = _cox_contact_line_liquid_force_map(state)
        else:
