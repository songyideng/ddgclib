...
# Eq. 3 pressure is evaluated with the v0 local quadratic neck fit.
USER_USE_CONSTRAINED_NECK_QUADRATIC = False
USER_ENABLE_NECK_FIT_RESIDUAL_SLOPE_PENALTY = False
...
USER_ENABLE_FORCE_DRIVEN_GMSH_CONTACT_LINE = True
USER_ENABLE_COX_CONTACT_LINE_FORCE = True
USER_COX_CONTACT_LINE_FORCE_SCALE = 1.0
...
def _sphere_normal_for_contact_vertex(
    vertex,
    *,
    state: VolumetricPitoisState,
) -> np.ndarray | None:
...
def _project_vector_to_contact_tangent(
    vector: np.ndarray,
    vertex,
    *,
    state: VolumetricPitoisState,
) -> np.ndarray:
    normal = _sphere_normal_for_contact_vertex(vertex, state=state)
    vec = np.asarray(vector, dtype=float)
    if normal is None:
        return vec
...
def _cox_contact_line_force_map(state: VolumetricPitoisState) -> dict[int, np.ndarray]:
    ...
    if (
        ...
        or (not bool(USER_ENABLE_COX_CONTACT_LINE_FORCE))
    ):
        state._cox_contact_line_force_cache = force_map
        return force_map

    _mid, axis, _e1, _e2 = _gmsh_axisym_basis(state)
    specs = (
    ...
    scale = float(USER_COX_CONTACT_LINE_FORCE_SCALE)
    for which, ring, sphere_center in specs:
        ...
        except (TypeError, ValueError, RuntimeError, np.linalg.LinAlgError, FloatingPointError):
            theta_geom = theta_eq
        ...
        theta_cox = _dynamic_contact_angle_from_speed(state, slide_speed=slide_speed)
        line_force_density = scale * gamma * (float(np.cos(theta_cox)) - float(np.cos(theta_geom)))
        seg_len = _ring_segment_length_map(ring)
        for vertex in ring:
            slide_dir = _contact_line_slide_direction(
                np.asarray(vertex.x_a[:3], dtype=float),
                sphere_center=sphere_center,
                axis=axis,
                which=which,
            )
            if float(np.linalg.norm(slide_dir)) <= 1.0e-30:
                continue
            force = line_force_density * float(seg_len.get(id(vertex), 0.0)) * slide_dir
            force_map[id(vertex)] = _project_vector_to_contact_tangent(force, vertex, state=state)
    state._cox_contact_line_force_cache = force_map
    return force_map


def _project_contact_line_force_map_to_sphere_tangent(
    state: VolumetricPitoisState,
    force_map: dict[int, np.ndarray],
) -> dict[int, np.ndarray]:
    if (
        (not bool(getattr(state, "gmsh_compute_mesh", False)))
        or (not bool(USER_ENABLE_FORCE_DRIVEN_GMSH_CONTACT_LINE))
    ):
        return force_map
    projected = dict(force_map)
    for vertex in state.bottom_contact_ring + state.top_contact_ring:
        vid = id(vertex)
        if vid not in projected:
            continue
        projected[vid] = _project_vector_to_contact_tangent(projected[vid], vertex, state=state)
    return projected


def _Fcl(v, *, state: VolumetricPitoisState) -> np.ndarray:
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        force = _cox_contact_line_force_map(state).get(id(v))
...
def _gmsh_axisym_force_map(
    ...
    *,
    ...
    averaged = _project_contact_line_force_map_to_sphere_tangent(state, averaged)
    cache[key] = averaged
