...
# Keep every force-history row, but do not redraw the live Fig. 5 comparison
# PNG on every step. The final comparison plot is still written from the full
# per-step history at the end of the run.
USER_FIG5_COMPARE_REFRESH_EVERY_STEPS = USER_RECORD_EVERY_STEPS
...
# After the divergence projection, correct the remaining total-volume error
# with a bounded local cell-volume balance solve. This catches late-stage cases
# where the kinematic scaling fallback cannot improve the .msh volume.
    ...
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        if bool(getattr(state.config, "enable_gmsh_geometric_volume_correction", False)):
            ...
            if (
                ...
                vertices = list(getattr(state, "volume_export_vertices", []))
                before_points = _volume_points_array(state).copy()
                before_error = abs(current - target)
                _axisym_force_snapshot_volume_to_target(
                    state,
                    rel_tol=float(state.config.volume_projection_rel_tol),
                    max_iters=max(40, int(state.config.volume_projection_max_iters)),
                )
                after = float(_snapshot_msh_volume_m3(state))
                if (
                    (not np.isfinite(after))
                    or after <= 0.0
                    or abs(after - target) >= before_error
                ):
                    _move_vertices_batch(
                        vertices,
                        [tuple(float(x) for x in row) for row in before_points],
                        state.HC,
                        state.bV_caps,
                    )
        _axisym_clear_caches(state)
...
def _apply_gmsh_flux_volume_constraint(
    ...
    *,
    ...
    for iteration in range(max(1, int(USER_FLUX_VOLUME_CONSTRAINT_MAX_ITERS))):
        ...
        _best_volume, best_rel, best_points = iteration_best
...
def _advance_gmsh_incompressible_substep(state: VolumetricPitoisState, *, dt: float) -> bool:
    ...
    if bool(getattr(state.config, "enable_gmsh_geometric_volume_correction", False)):
        ...
        if (
            ...
            and abs(current_volume - target) / max(target, 1.0e-30) > trigger
        ):
            rel_before = float((current_volume - target) / max(target, 1.0e-30))
            state.last_position_volume_constraint_rel_before = rel_before
            state.last_position_volume_constraint_rel_after = rel_before
            ...
            state.last_position_volume_constraint_rel_after = rel_after
            state.last_gmsh_cl_volume_slide_rel_after = rel_after
            if abs(rel_after) <= max(float(state.config.volume_projection_rel_tol), 1.0e-12):
                state.last_position_volume_constraint_status = "geometric_applied"
            elif abs(rel_after) < abs(rel_before):
                state.last_position_volume_constraint_status = "geometric_best_effort"
            else:
                state.last_position_volume_constraint_status = "geometric_no_improvement"
            _gmsh_set_velocities_from_displacement(
                state,
                reference_positions=substep_reference_positions,
                dt=dt,
            )
...
def run_motion_case(
    ...
    *,
    ...
    for step in range(config.n_steps):
        ...
        if save_fig:
            fig5_refresh_every = max(1, int(USER_FIG5_COMPARE_REFRESH_EVERY_STEPS))
            if (
                completed_step == 1
                or completed_step == int(config.n_steps)
                or completed_step % fig5_refresh_every == 0
            ):
                _refresh_pitois_fig5_compare_png(
                    separation_history=history,
                    out_dir=out_dir,
