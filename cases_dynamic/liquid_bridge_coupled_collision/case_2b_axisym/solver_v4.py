"""Computation-only solver for Case 2b Gmsh Pitois-2000 separation v4.
...
``Case_2b_axisym_pitois2000_separation_Gmsh v4.py``.
"""
...
# Diagnostic neck curvature fit.  This is not the force-path p0; the force-path
# p0 is solved from the fixed-volume force residual.
USER_P0_MERIDIAN_FIT_HALF_WINDOW_RINGS = max(3, USER_NECK_EXTRA_AXIAL_LAYERS // 2)
USER_P0_MERIDIAN_FIT_POLY_DEGREE = 2
# Fixed-D pre-equilibration before the 5 um/s separation run.  The spheres are
# held fixed, the liquid volume is projected back to target each iteration, and
# the free liquid interface/contact line relax through the same Stokes/continuity
# and Heron/Cox force path used by the separation run.
USER_PRE_EQUILIBRATE_INITIAL_SHAPE = True
USER_PRE_EQUILIBRATION_STEPS = 20
USER_PRE_EQUILIBRATION_DT_S = USER_DT_S
...
class VolumetricPitoisState:
    ...
    p0_solved_pa: float = 0.0
    p0_reference_pa: float = 0.0
    ...
    hold_spheres_fixed: bool = False
    use_incremental_stokes_force_load: bool = True
    pre_equilibrated_initial_shape: bool = False
...
def separation_config() -> VolumetricPitoisConfig:
    return VolumetricPitoisConfig(
        name="Case_2b_axisym_pitois2000_separation_Gmsh v4",
        title="Case 2b axisym: volumetric pre-bridged separation",
    )


def _sphere_velocity_pair(state: VolumetricPitoisState) -> tuple[np.ndarray, np.ndarray]:
    """Return bottom/top sphere velocities used by this step."""
    if bool(getattr(state, "hold_spheres_fixed", False)):
        return np.zeros(3, dtype=float), np.zeros(3, dtype=float)
    return np.array([0.0, 0.0, -float(state.config.cap_speed)], dtype=float), np.zeros(3, dtype=float)


def _sphere_relative_speed(state: VolumetricPitoisState) -> float:
    bottom_velocity, top_velocity = _sphere_velocity_pair(state)
    _bottom_center, _top_center, axis = _particle_centers_physical(state)
    return float(np.dot(top_velocity - bottom_velocity, axis))
...
def _lubrication_pressure_diagnostics(state: VolumetricPitoisState) -> SimpleNamespace:
    ...
    separation_speed = abs(float(_sphere_relative_speed(state)))
...
def _side_dual_areas_and_normals(
    state: VolumetricPitoisState,
) -> tuple[list[object], dict[int, float], dict[int, np.ndarray]]:
    side_triangles = _actual_compute_side_surface_triangles(state)
    if side_triangles.size == 0:
        return [], {}, {}
...
def _side_area_normal_maps_cached(
    state: VolumetricPitoisState,
) -> tuple[list[object], dict[int, float], dict[int, np.ndarray]]:
    cached = getattr(state, "_side_area_normal_cache", None)
    if cached is not None:
        return cached
    cached = _side_dual_areas_and_normals(state)
...
def _p0_volume_constraint_from_force_map(
    state: VolumetricPitoisState,
    force_map: dict[int, np.ndarray],
) -> float:
    """Solve the scalar fixed-volume pressure offset from the force residual."""
    vertices, area_map, normal_map = _side_area_normal_maps_cached(state)
    numerator = 0.0
    denominator = 0.0
    for v in vertices:
        area = float(area_map.get(id(v), 0.0))
        ...
        if area <= 0.0 or normal is None:
            continue
        volume_gradient = area * np.asarray(normal, dtype=float)
        source_force = np.asarray(force_map.get(id(v), np.zeros(3, dtype=float)), dtype=float)
        numerator += float(np.dot(volume_gradient, source_force))
        denominator += float(np.dot(volume_gradient, volume_gradient))
    if denominator <= 1.0e-30 or not np.isfinite(numerator):
        return 0.0
    # Code force convention uses Fp = p A n.  This scalar is the pressure
    # offset that removes the source-force component along the volume gradient.
...
def _initialize_stokes_reference_force_load(state: VolumetricPitoisState) -> None:
    ...
    state.p0_reference_pa = float(_p0_volume_constraint_from_force_map(state, raw_map))
    state.p0_solved_pa = float(state.p0_reference_pa)
...
def _stokes_force_load_map(state: VolumetricPitoisState) -> dict[int, np.ndarray]:
    ...
    raw_map = _raw_stokes_force_load_map(state)
    if not bool(getattr(state, "use_incremental_stokes_force_load", True)):
        return {vertex_id: np.asarray(load, dtype=float) for vertex_id, load in raw_map.items()}
...
def _solve_stokes_continuity_flow(state: VolumetricPitoisState, *, dt: float) -> bool:
    ...
    p0_col = n_velocity + n_vertices
    n_unknowns = n_velocity + n_vertices + 1
    coords = np.asarray([np.asarray(v.x_a[:3], dtype=float) for v in vertices], dtype=float)
    ...
    raw_force_load_map = _raw_stokes_force_load_map(state)
    p0_absolute_target = float(_p0_volume_constraint_from_force_map(state, raw_force_load_map))
    if bool(getattr(state, "use_incremental_stokes_force_load", True)):
        p0_unknown_target = p0_absolute_target - float(getattr(state, "p0_reference_pa", 0.0))
    else:
        p0_unknown_target = p0_absolute_target
    ...
    _boundary_vertices, boundary_area_map, boundary_normal_map = _boundary_area_normal_maps_cached(state)
    p0_row = p0_col
    for vertex_id, area in boundary_area_map.items():
        idx = id_to_idx.get(vertex_id)
        normal = boundary_normal_map.get(vertex_id)
        if idx is None or normal is None:
            continue
        area = float(area)
        normal = np.asarray(normal, dtype=float)
        if area <= 0.0 or not np.all(np.isfinite(normal)):
            continue
        traction_vector = area * normal
        for i_dim in range(3):
            coeff = float(traction_vector[i_dim])
            if abs(coeff) <= 1.0e-30:
                continue
            # Uniform liquid-pressure offset p0.  This is the global pressure
            # Lagrange multiplier that enters the Stokes residual, not a
            # post-step fitted curvature diagnostic.
            matrix[3 * idx + i_dim, p0_col] += -coeff
    matrix[p0_row, p0_col] = 1.0
    rhs[p0_row] = float(p0_unknown_target)
    ...
    solved_pressure = solution[n_velocity:p0_col]
    solved_pressure = solved_pressure + float(getattr(state.config, "continuity_pressure_pa", 0.0))
    solved_p0_unknown = float(solution[p0_col])
    if bool(getattr(state, "use_incremental_stokes_force_load", True)):
        solved_p0 = float(getattr(state, "p0_reference_pa", 0.0)) + solved_p0_unknown
    else:
        solved_p0 = solved_p0_unknown
...
def _pressure_model(v, *, HC=None, dim: int = 3, state: VolumetricPitoisState):
    ...
    pressure += float(getattr(state, "p0_solved_pa", 0.0))
...
def _set_cap_velocities(state: VolumetricPitoisState) -> None:
    bottom_u, top_u = _sphere_velocity_pair(state)
    for v in state.cap_bottom_interior:
        v.u = bottom_u.copy()
    for v in state.cap_top_interior:
        v.u = top_u.copy()
...
def _base_move_caps(state: VolumetricPitoisState, *, dt: float | None = None) -> None:
    ...
    bottom_vel, top_vel = _sphere_velocity_pair(state)
...
def _update_moving_contact_line(state: VolumetricPitoisState, *, dt: float | None = None) -> None:
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        ...
        _align_layer_azimuths(state)
        _regularize_layer_order(state)
        bottom_sphere_velocity, top_sphere_velocity = _sphere_velocity_pair(state)
        slide_map: dict[int, float] = {}
...
def _solver_top_sphere_force_components_n(state: VolumetricPitoisState) -> SimpleNamespace:
    ...
    # The local Gmsh viscous map is shear only; add the speed-dependent
    # lubrication-pressure part of the Stokes stress to the sphere force.
    top_sphere_Fv_n += float(_lubrication_pressure_diagnostics(state).force_n)
...
def _top_contact_line_diagnostics(state: VolumetricPitoisState) -> SimpleNamespace:
    ...
    for v in ring:
        ...
        Fv = force_terms.Fv
        Fcl = force_terms.Fcl
        sum_Fp_n += float(np.dot(Fp, axis))
        sum_Fs_n += float(np.dot(Fs, axis))
        sum_Fv_n += float(np.dot(Fv, axis))
        sum_Fcl_n += float(np.dot(Fcl, axis))

    count = max(len(ring), 1)
    avg_Fp_n = sum_Fp_n / count
    avg_Fs_n = sum_Fs_n / count
    avg_Fv_n = sum_Fv_n / count
    ...
    return SimpleNamespace(
        ...
        avg_Ftot_n=float(avg_Fp_n + avg_Fs_n + avg_Fv_n + avg_Fcl_n),
    )
...
def _step_record(state: VolumetricPitoisState, *, step: int, t: float) -> dict[str, float | int]:
    ...
    p0_solved_pa = float(getattr(state, "p0_solved_pa", 0.0))
    _boundary_vertices, cap_area_map, cap_normal_map = _boundary_area_normal_maps_cached(state)
    _bottom_center, _top_center, axis = _particle_centers_physical(state)
    top_cap_p0_solved_force_n = 0.0
    for v in state.cap_top:
        area = float(cap_area_map.get(id(v), 0.0))
        normal = cap_normal_map.get(id(v))
        if area <= 0.0 or normal is None:
            continue
        top_cap_p0_solved_force_n += p0_solved_pa * area * float(np.dot(normal, axis))
    ...
    return {
        ...
        "p0_solved_pa": p0_solved_pa,
        "Ftopspherecap_p0_solved_n": float(top_cap_p0_solved_force_n),
        "Ftopspherecap_p0_solved_mN": 1.0e3 * float(top_cap_p0_solved_force_n),
        ...
        "p0_neck_fit_H_avg_1pm": float(p0_diagnostics.H_avg_1pm),
...
def _clear_fixed_d_relaxation_velocity(state: VolumetricPitoisState) -> None:
    """Remove pseudo-time relaxation velocities before static force reporting."""
    ...
    state.last_bottom_contact_line_speed = 0.0
    state.last_top_contact_line_speed = 0.0
    state.last_top_cl_avg_ucl = zero.copy()
    state.last_top_cl_avg_dxyz = zero.copy()
    _axisym_clear_caches(state)


def _pre_equilibration_iteration(state: VolumetricPitoisState, *, dt: float) -> None:
    _set_cap_velocities(state)
    _enforce_no_swirl_velocity_field(state)
    _gmsh_axisymmetrize_state(state)
    _update_duals_and_masses(state)
    _update_pressure_scalar(state)
    _advance_free_vertices_from_stokes_flow(state, dt=dt)
    _gmsh_axisymmetrize_state(state)
    _enforce_no_swirl_velocity_field(state)
    _set_cap_velocities(state)
    ...
    _update_duals_and_masses(state)
    _update_pressure_scalar(state)
    if not _solve_stokes_continuity_flow(state, dt=dt):
        raise RuntimeError("Stokes/continuity flow solve failed during fixed-D pre-equilibration.")
    _gmsh_axisymmetrize_state(state)
    _enforce_no_swirl_velocity_field(state)
    _clear_fixed_d_relaxation_velocity(state)
    _update_top_sphere_force_n(state)
    _assert_fixed_topology(state)


def pre_equilibrate_initial_shape(
    state: VolumetricPitoisState,
    *,
    steps: int | None = None,
    dt: float | None = None,
    ...
    """Relax the imported t=0 mesh at fixed sphere separation and fixed volume."""
    n_steps = int(USER_PRE_EQUILIBRATION_STEPS if steps is None else steps)
    if not bool(USER_PRE_EQUILIBRATE_INITIAL_SHAPE) or n_steps <= 0:
        _initialize_stokes_reference_force_load(state)
        return []
    step_dt = float(USER_PRE_EQUILIBRATION_DT_S if dt is None else dt)
    if step_dt <= 0.0:
        step_dt = float(state.config.dt if state.config.dt > 0.0 else 1.0e-6)

    old_hold = bool(getattr(state, "hold_spheres_fixed", False))
    old_incremental = bool(getattr(state, "use_incremental_stokes_force_load", True))
    old_elapsed = float(getattr(state, "elapsed_time_s", 0.0))
    old_limiter = str(getattr(state, "last_dt_limiter", "fixed"))
    old_dt = float(getattr(state, "last_step_dt", step_dt))

    state.hold_spheres_fixed = True
    state.use_incremental_stokes_force_load = False
    state.last_dt_limiter = "fixed_D_pre_equilibration"
    state.last_step_dt = float(step_dt)

    history: list[dict[str, float | int]] = []
    try:
        for step in range(n_steps):
            _pre_equilibration_iteration(state, dt=step_dt)
            _assert_state_finite(state, where=f"fixed-D pre-equilibration {step + 1}")
            record = _step_record(state, step=step + 1, t=0.0)
            history.append(record)
            if verbose:
                print(
                    "Fixed-D pre-equilibration = "
                    f"{step + 1}/{n_steps}, "
                    f"top_sphere_force_n {1.0e3 * float(record['top_sphere_force_n']):+.6e} mN, "
                    f"water vol. error {float(record['water_vol_error_percent']):+.4f} %",
                    flush=True,
                )
        state.pre_equilibrated_initial_shape = True
    finally:
        state.hold_spheres_fixed = old_hold
        state.use_incremental_stokes_force_load = old_incremental
        state.elapsed_time_s = old_elapsed
        state.last_dt_limiter = old_limiter
        state.last_step_dt = old_dt
    ...
    _update_pressure_scalar(state)
    _initialize_stokes_reference_force_load(state)
    if not _solve_stokes_continuity_flow(state, dt=max(float(state.config.dt), 1.0e-12)):
        raise RuntimeError("Stokes/continuity flow solve failed after fixed-D pre-equilibration.")
    _gmsh_axisymmetrize_state(state)
    _enforce_no_swirl_velocity_field(state)
    _update_top_sphere_force_n(state)
    return history
...
__all__ = [
    "USER_REFINEMENT",
    "USER_TOTAL_STEPS",
    "USER_PRE_EQUILIBRATE_INITIAL_SHAPE",
    "USER_PRE_EQUILIBRATION_STEPS",
    "USER_PRE_EQUILIBRATION_DT_S",
