...
except Exception:  # pragma: no cover - Matplotlib savefig fallback remains available.
    Image = None
...
# Max separation iterations for the full Fig. 5 sweep.
...
USER_VERBOSE_EVERY_STEPS = USER_RECORD_EVERY_STEPS
USER_FAST_FIG5_REFRESH = True
USER_FAST_TET_GEOMETRY_BATCH = True
...
USER_MAX_VOLUME_REL_ERROR_FOR_ABORT = 0.05
...
# Post-coordinate hard volume restore for the Gmsh incompressible path. This
# runs after Ftot/CL forces, velocity projection, and geometric constraints, so
# the force model and Fig. 5 plotting code remain the v5 optimized path.
USER_ENABLE_KINEMATIC_VOLUME_CONSTRAINT = True
USER_KINEMATIC_VOLUME_CONSTRAINT_REL_TOL = 1.0e-6
...
# v7: after the divergence projection, correct the remaining total-volume
# error with a local cell-volume balance solve. This is not per-tet Lagrangian
# volume locking: individual tetra volumes may change, but the shared-node
# correction carries the global residual through the tetrahedral mesh before
# the v6 kinematic scaling fallback is allowed to run.
USER_ENABLE_FLUX_VOLUME_CONSTRAINT = True
USER_FLUX_VOLUME_CONSTRAINT_REL_TOL = 1.0e-6
USER_FLUX_VOLUME_CONSTRAINT_MAX_ITERS = 8
USER_FLUX_VOLUME_CONSTRAINT_EDGE_FRACTION = 1.5
USER_FLUX_VOLUME_CONSTRAINT_MAX_CORRECTION_M = 1.0e-4
USER_FLUX_VOLUME_CONSTRAINT_REGULARIZATION = 1.0e-10
# v8: maintain conserved vertex control-volume values and update them with
# antisymmetric ALE fluxes through DDG dual-area vectors A_ij. The DDG duals are
# computed only inside this flux update, then cleared, so the existing v6/v7
# force path is not changed by accidentally enabling the base DDG stress branch.
USER_ENABLE_ALE_DDG_VERTEX_MASS_FLUX = True
USER_ALE_VERTEX_MASS_MIN_FRACTION = 1.0e-3
USER_ALE_VERTEX_MASS_RENORMALIZE_TOTAL = True
USER_ALE_VERTEX_MASS_GEOMETRY_FLOOR_FRACTION = 0.25
USER_ALE_VERTEX_MASS_REFERENCE_FLOOR_FRACTION = 0.05
USER_ALE_VERTEX_MASS_GEOMETRY_CEILING_FACTOR = 4.0
USER_ALE_DDG_VERTEX_FLUX_BATCH_DUALS = True
USER_ALE_DDG_VERTEX_FLUX_CACHE_EDGES = True
USER_ENABLE_ALE_DDG_VERTEX_MOMENTUM_FLUX = True
USER_ALE_DDG_VERTEX_MOMENTUM_FACE_SCHEME = "upwind"  # "upwind" or "central"
USER_ENABLE_ALE_DDG_POST_MOMENTUM_PROJECTION = True
# The conservative ALE momentum state is updated with the same DDG dual-area
# flux as mass, but it must not directly overwrite the mesh velocity unless a
# fully stable ALE momentum solve is being tested. Direct overwrite was the
# source of the long-run mesh/force oscillations in v9.
USER_ALE_DDG_MOMENTUM_APPLY_TO_MESH_VELOCITY = False
USER_ALE_DDG_MOMENTUM_VELOCITY_RELAXATION = 0.0
...
class VolumetricPitoisState:
    ...
    last_flux_volume_constraint_status: str = "not_run"
    last_flux_volume_constraint_rel_before: float = 0.0
    last_flux_volume_constraint_rel_after: float = 0.0
    last_flux_volume_constraint_iterations: int = 0
    ale_vertex_volumes_m3: np.ndarray = field(default_factory=lambda: np.empty(0, dtype=float))
    ale_vertex_reference_volumes_m3: np.ndarray = field(default_factory=lambda: np.empty(0, dtype=float))
    ale_vertex_total_volume_m3: float = 0.0
    ale_vertex_momentum_m3_mps: np.ndarray = field(default_factory=lambda: np.empty((0, 3), dtype=float))
    last_ale_vertex_mass_flux_status: str = "not_run"
    last_ale_vertex_mass_flux_total_rel: float = 0.0
    last_ale_vertex_mass_flux_max_rel_delta: float = 0.0
    last_ale_vertex_mass_flux_min_fraction: float = 1.0
    last_ale_vertex_mass_flux_clipped: int = 0
    last_ale_vertex_mass_flux_limited_edges: int = 0
    last_ale_vertex_mass_flux_edges: int = 0
    last_ale_vertex_mass_flux_skipped_edges: int = 0
    last_ale_vertex_momentum_flux_status: str = "not_run"
    last_ale_vertex_momentum_flux_total_rel: float = 0.0
    last_ale_vertex_momentum_flux_post_constraint_rel: float = 0.0
    last_ale_vertex_momentum_flux_max_rel_delta: float = 0.0
    last_ale_vertex_momentum_flux_max_speed: float = 0.0
...
def _gmsh_tet_cell_volumes(points: np.ndarray, tets: np.ndarray) -> np.ndarray:
    pts = np.asarray(points, dtype=float)
    ...
    return np.nan_to_num(volumes, nan=0.0, posinf=0.0, neginf=0.0)


def _gmsh_lump_cell_volumes_to_vertices(
    *,
    n_vertices: int,
    tets: np.ndarray,
    cell_volumes: np.ndarray,
) -> np.ndarray:
    lumped = np.zeros(int(n_vertices), dtype=float)
    tet_idx = np.asarray(tets, dtype=int)
    volumes = np.asarray(cell_volumes, dtype=float)
    if tet_idx.size == 0 or volumes.size != int(tet_idx.shape[0]):
        return lumped
    share = 0.25 * volumes
    for local in range(4):
        np.add.at(lumped, tet_idx[:, local], share)
    return lumped


def _gmsh_ensure_ale_vertex_mass_state(
    state: VolumetricPitoisState,
    *,
    geometry_vertex_volumes: np.ndarray,
) -> None:
    geometry = np.asarray(geometry_vertex_volumes, dtype=float)
    n_vertices = int(geometry.size)
    current = np.asarray(getattr(state, "ale_vertex_volumes_m3", np.empty(0, dtype=float)), dtype=float)
    if current.size == n_vertices and n_vertices > 0 and np.all(np.isfinite(current)):
        return
    if n_vertices <= 0:
        state.ale_vertex_volumes_m3 = np.empty(0, dtype=float)
        state.ale_vertex_reference_volumes_m3 = np.empty(0, dtype=float)
        state.ale_vertex_total_volume_m3 = 0.0
        return
    geometry = np.maximum(np.nan_to_num(geometry, nan=0.0, posinf=0.0, neginf=0.0), 0.0)
    geometry_sum = float(np.sum(geometry))
    target = float(getattr(state, "target_snapshot_volume_m3", 0.0))
    if (not np.isfinite(target)) or target <= 0.0:
        target = geometry_sum
    if geometry_sum > 1.0e-30 and target > 0.0:
        conserved = geometry * (target / geometry_sum)
    else:
        conserved = np.full(n_vertices, max(target, 1.0e-30) / float(n_vertices), dtype=float)
    conserved = np.maximum(conserved, 1.0e-30)
    state.ale_vertex_volumes_m3 = conserved.copy()
    state.ale_vertex_reference_volumes_m3 = conserved.copy()
    state.ale_vertex_total_volume_m3 = float(np.sum(conserved))
    state.last_ale_vertex_mass_flux_status = "initialized"
    state.last_ale_vertex_mass_flux_total_rel = 0.0
    state.last_ale_vertex_mass_flux_max_rel_delta = 0.0
    state.last_ale_vertex_mass_flux_min_fraction = 1.0
    state.last_ale_vertex_mass_flux_clipped = 0
    state.last_ale_vertex_mass_flux_limited_edges = 0
    state.last_ale_vertex_mass_flux_edges = 0
    state.last_ale_vertex_mass_flux_skipped_edges = 0
    state.last_ale_vertex_momentum_flux_status = "not_run"
    state.last_ale_vertex_momentum_flux_total_rel = 0.0
    state.last_ale_vertex_momentum_flux_post_constraint_rel = 0.0
    state.last_ale_vertex_momentum_flux_max_rel_delta = 0.0
    state.last_ale_vertex_momentum_flux_max_speed = 0.0


def _gmsh_ensure_ale_vertex_momentum_state(
    state: VolumetricPitoisState,
    *,
    conserved_vertex_volumes: np.ndarray,
    velocities: np.ndarray,
) -> None:
    volumes = np.asarray(conserved_vertex_volumes, dtype=float)
    vel = np.asarray(velocities, dtype=float)
    n_vertices = int(volumes.size)
    current = np.asarray(getattr(state, "ale_vertex_momentum_m3_mps", np.empty((0, 3), dtype=float)), dtype=float)
    if current.shape == (n_vertices, 3) and n_vertices > 0 and np.all(np.isfinite(current)):
        return
    if n_vertices <= 0 or vel.shape != (n_vertices, 3):
        state.ale_vertex_momentum_m3_mps = np.empty((0, 3), dtype=float)
        state.last_ale_vertex_momentum_flux_status = "bad_init"
        return
    vel = np.nan_to_num(vel, nan=0.0, posinf=0.0, neginf=0.0)
    state.ale_vertex_momentum_m3_mps = np.maximum(volumes, 1.0e-30)[:, None] * vel
    state.last_ale_vertex_momentum_flux_status = "initialized"
    state.last_ale_vertex_momentum_flux_total_rel = 0.0
    state.last_ale_vertex_momentum_flux_post_constraint_rel = 0.0
    state.last_ale_vertex_momentum_flux_max_rel_delta = 0.0
    state.last_ale_vertex_momentum_flux_max_speed = float(np.max(np.linalg.norm(vel, axis=1))) if vel.size else 0.0


def _gmsh_sync_ale_vertex_momentum_from_velocities(
    state: VolumetricPitoisState,
    *,
    conserved_vertex_volumes: np.ndarray,
) -> None:
    vertices = list(getattr(state, "volume_export_vertices", []))
    volumes = np.asarray(conserved_vertex_volumes, dtype=float)
    if len(vertices) != int(volumes.size):
        return
    ...
    state.ale_vertex_momentum_m3_mps = np.maximum(volumes, 1.0e-30)[:, None] * velocities


def _gmsh_constrained_velocity_array(
    state: VolumetricPitoisState,
    velocities: np.ndarray,
    *,
    vertices: list,
) -> np.ndarray:
    constrained = np.asarray(velocities, dtype=float).copy()
    if constrained.shape != (len(vertices), 3):
        return np.zeros((len(vertices), 3), dtype=float)
    constrained = np.nan_to_num(constrained, nan=0.0, posinf=0.0, neginf=0.0)

    if bool(getattr(state.config, "enforce_no_swirl", False)):
        axis_origin, axis = _swirl_axis_geometry(state)
        for idx, vertex in enumerate(vertices):
            constrained[idx] = _remove_swirl_component(
                constrained[idx],
    ...
    index_by_id = {id(vertex): idx for idx, vertex in enumerate(vertices)}
    bottom_u = np.array([0.0, 0.0, -float(state.config.cap_speed)], dtype=float)
    top_u = np.zeros(3, dtype=float)
    for vertex in state.cap_bottom_interior:
        idx = index_by_id.get(id(vertex))
        if idx is not None:
            constrained[int(idx)] = bottom_u
    for vertex in state.cap_top_interior:
        idx = index_by_id.get(id(vertex))
        if idx is not None:
            constrained[int(idx)] = top_u

    for ring, sphere_center, sphere_velocity in (
        (state.bottom_contact_ring, np.asarray(state.bottom_sphere_center, dtype=float), bottom_u),
        (state.top_contact_ring, np.asarray(state.top_sphere_center, dtype=float), top_u),
    ):
        for vertex in ring:
            idx = index_by_id.get(id(vertex))
            if idx is None:
                continue
            idx = int(idx)
            normal = np.asarray(vertex.x_a[:3], dtype=float) - sphere_center
            normal_norm = float(np.linalg.norm(normal))
            if normal_norm <= 1.0e-30:
                continue
            normal = normal / normal_norm
            rel_u = constrained[idx] - sphere_velocity
            constrained[idx] = sphere_velocity + rel_u - float(np.dot(rel_u, normal)) * normal

    return constrained


def _gmsh_ale_vertex_volume_bounds(
    state: VolumetricPitoisState,
    *,
    geometry_vertex_volumes: np.ndarray,
    reference_vertex_volumes: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    reference = np.asarray(reference_vertex_volumes, dtype=float)
    geometry = np.asarray(geometry_vertex_volumes, dtype=float)
    if reference.size == 0:
        return np.empty(0, dtype=float), np.empty(0, dtype=float)
    reference = np.maximum(
        np.nan_to_num(reference, nan=0.0, posinf=0.0, neginf=0.0),
        1.0e-30,
    )
    if geometry.size != reference.size:
        ...
        geometry = np.maximum(
            np.nan_to_num(geometry, nan=0.0, posinf=0.0, neginf=0.0),
            1.0e-30,
        )

    floor = max(float(USER_ALE_VERTEX_MASS_MIN_FRACTION), 0.0) * reference
    floor = np.maximum(
        floor,
        max(float(USER_ALE_VERTEX_MASS_REFERENCE_FLOOR_FRACTION), 0.0) * reference,
    )
    floor = np.maximum(
        floor,
        max(float(USER_ALE_VERTEX_MASS_GEOMETRY_FLOOR_FRACTION), 0.0) * geometry,
    )
    floor = np.maximum(floor, 1.0e-30)

    target_total = float(getattr(state, "ale_vertex_total_volume_m3", 0.0))
    if (not np.isfinite(target_total)) or target_total <= 0.0:
        target_total = float(np.sum(reference))
    floor_sum = float(np.sum(floor))
    if floor_sum >= 0.95 * max(target_total, 1.0e-30):
        floor *= (0.95 * target_total) / max(floor_sum, 1.0e-30)

    ceiling_factor = max(float(USER_ALE_VERTEX_MASS_GEOMETRY_CEILING_FACTOR), 1.0)
    ceiling = ceiling_factor * geometry
    ceiling = np.maximum(ceiling, floor)
    return floor, ceiling


def _gmsh_renormalize_ale_vertex_volumes(
    state: VolumetricPitoisState,
    volumes: np.ndarray,
    *,
    ...
    values = np.asarray(volumes, dtype=float).copy()
    if values.size == 0:
        return values, 0
    reference = np.asarray(getattr(state, "ale_vertex_reference_volumes_m3", np.empty(0, dtype=float)), dtype=float)
    if reference.size != values.size:
        reference = np.maximum(values.copy(), 1.0e-30)
    target_total = float(getattr(state, "ale_vertex_total_volume_m3", 0.0))
    if (not np.isfinite(target_total)) or target_total <= 0.0:
        target_total = float(np.sum(reference))
    min_fraction = max(float(USER_ALE_VERTEX_MASS_MIN_FRACTION), 0.0)
    floor = min_fraction * np.maximum(reference, 1.0e-30)
    if lower_bounds is not None:
        lower = np.asarray(lower_bounds, dtype=float)
        if lower.size == values.size:
            lower = np.maximum(
                np.nan_to_num(lower, nan=0.0, posinf=0.0, neginf=0.0),
                1.0e-30,
            )
            floor = np.maximum(floor, lower)
    floor_sum = float(np.sum(floor))
    if floor_sum >= 0.95 * max(target_total, 1.0e-30):
        floor *= (0.95 * target_total) / max(floor_sum, 1.0e-30)

    ceiling = np.full_like(values, np.inf, dtype=float)
    if upper_bounds is not None:
        upper = np.asarray(upper_bounds, dtype=float)
        if upper.size == values.size:
            upper = np.nan_to_num(
                upper,
                nan=np.inf,
                posinf=np.inf,
            ...
            ceiling = np.maximum(upper, floor)

    original_values = values.copy()
    clipped = int(
        np.count_nonzero(
            (~np.isfinite(original_values))
            | (original_values < floor)
            | (original_values > ceiling)
        )
    )
    values = np.nan_to_num(values, nan=0.0, posinf=np.inf, neginf=0.0)
    values = np.minimum(np.maximum(values, floor), ceiling)
    if not bool(USER_ALE_VERTEX_MASS_RENORMALIZE_TOTAL):
        return values, clipped

    current_total = float(np.sum(values))
    if (not np.isfinite(current_total)) or current_total <= 0.0:
        values = np.minimum(np.maximum(reference.copy(), floor), ceiling)
        current_total = float(np.sum(values))
    floor_total = float(np.sum(floor))
    ceiling_total = float(np.sum(ceiling))
    effective_target = max(target_total, floor_total)
    if np.isfinite(ceiling_total):
        effective_target = min(effective_target, ceiling_total)
    if abs(current_total - effective_target) <= max(1.0e-14 * effective_target, 1.0e-30):
        return values, clipped
    if current_total > effective_target:
        for _ in range(4):
            surplus = np.maximum(values - floor, 0.0)
            surplus_sum = float(np.sum(surplus))
            remove = float(np.sum(values)) - effective_target
            if remove <= max(1.0e-14 * effective_target, 1.0e-30) or surplus_sum <= 1.0e-30:
                break
            values -= surplus * min(remove / surplus_sum, 1.0)
            values = np.minimum(np.maximum(values, floor), ceiling)
    else:
        for _ in range(6):
            current_total = float(np.sum(values))
            deficit = effective_target - current_total
            if deficit <= max(1.0e-14 * effective_target, 1.0e-30):
                break
            capacity = np.maximum(ceiling - values, 0.0)
            active = capacity > 1.0e-30
            if not np.any(active):
                break
            weights = np.where(active, np.maximum(reference, 1.0e-30), 0.0)
            weight_sum = float(np.sum(weights))
            if weight_sum <= 1.0e-30:
                break
            add = deficit * weights / weight_sum
...
def _gmsh_clear_transient_ddg_duals(state: VolumetricPitoisState) -> None:
    for vertex in getattr(state, "volume_export_vertices", []):
        if hasattr(vertex, "vd"):
            try:
                delattr(vertex, "vd")
...
def _gmsh_recompute_ddg_duals_for_flux(state: VolumetricPitoisState) -> None:
    state.HC.dim = 3
    if not bool(USER_ALE_DDG_VERTEX_FLUX_BATCH_DUALS):
        _recompute_duals(state.HC)
        return
    try:
        ...
        compute_vd(state.HC, method="barycentric", backend=get_backend("numpy"))
    except Exception:
        _recompute_duals(state.HC)
...
def _update_duals_and_masses(state: VolumetricPitoisState) -> None:
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        if hasattr(state, "_dual_area_vector_cache"):
            delattr(state, "_dual_area_vector_cache")
        _gmsh_clear_transient_ddg_duals(state)
        vertices = list(state.volume_export_vertices)
        points = np.asarray([np.asarray(v.x_a[:3], dtype=float) for v in vertices], dtype=float)
        tets = np.asarray(state.volume_export_tets, dtype=int)
        geometry_cell_volumes = _gmsh_tet_cell_volumes(points, tets)
        geometry_lumped = _gmsh_lump_cell_volumes_to_vertices(
            n_vertices=len(vertices),
            tets=tets,
            cell_volumes=geometry_cell_volumes,
        )
        if bool(USER_ENABLE_ALE_DDG_VERTEX_MASS_FLUX):
            _gmsh_ensure_ale_vertex_mass_state(state, geometry_vertex_volumes=geometry_lumped)
            conserved_vertex_volumes = np.asarray(state.ale_vertex_volumes_m3, dtype=float)
            if conserved_vertex_volumes.size != len(vertices):
                conserved_vertex_volumes = geometry_lumped
            lumped = conserved_vertex_volumes
        else:
            state.last_ale_vertex_mass_flux_status = "disabled"
            lumped = geometry_lumped
        for vertex, mass_volume, geometry_volume in zip(vertices, lumped, geometry_lumped):
            vertex.m = max(float(mass_volume), 1.0e-12)
            vertex.geometric_m = max(float(geometry_volume), 1.0e-12)
        _axisym_clear_caches(state)
        return
...
def _apply_gmsh_ale_ddg_vertex_mass_flux(
    state: VolumetricPitoisState,
    *,
    dt: float,
    ...
    state.last_ale_vertex_mass_flux_status = "not_run"
    state.last_ale_vertex_mass_flux_total_rel = 0.0
    state.last_ale_vertex_mass_flux_max_rel_delta = 0.0
    state.last_ale_vertex_mass_flux_min_fraction = 1.0
    state.last_ale_vertex_mass_flux_clipped = 0
    ...
    if (
        ...
        or (not bool(USER_ENABLE_ALE_DDG_VERTEX_MASS_FLUX))
        or float(dt) <= 0.0
    ):
        state.last_ale_vertex_mass_flux_status = "disabled"
        return
    ...
    if (not vertices) or tets.size == 0:
        state.last_ale_vertex_mass_flux_status = "empty_mesh"
        return

    current_points = _volume_points_array(state)
    reference = np.asarray(reference_positions, dtype=float)
    fluid_u = np.asarray(fluid_velocities, dtype=float)
    if reference.shape != current_points.shape or fluid_u.shape != current_points.shape:
        state.last_ale_vertex_mass_flux_status = "bad_reference"
        return
    if (
        (not np.all(np.isfinite(reference)))
        or (not np.all(np.isfinite(current_points)))
        or (not np.all(np.isfinite(fluid_u)))
    ):
        state.last_ale_vertex_mass_flux_status = "bad_points"
        return

    geometry_cell_volumes = _gmsh_tet_cell_volumes(current_points, tets)
    geometry_lumped = _gmsh_lump_cell_volumes_to_vertices(
        n_vertices=len(vertices),
        tets=tets,
        cell_volumes=geometry_cell_volumes,
    ...
    _gmsh_ensure_ale_vertex_mass_state(state, geometry_vertex_volumes=geometry_lumped)
    conserved = np.asarray(getattr(state, "ale_vertex_volumes_m3", np.empty(0, dtype=float)), dtype=float)
    if conserved.size != len(vertices):
        state.last_ale_vertex_mass_flux_status = "bad_vertex_state"
        return
    reference_vertices = np.asarray(
        getattr(state, "ale_vertex_reference_volumes_m3", np.empty(0, dtype=float)),
        dtype=float,
    )
    if reference_vertices.size != conserved.size:
        reference_vertices = np.maximum(conserved.copy(), 1.0e-30)
    volume_floor, volume_ceiling = _gmsh_ale_vertex_volume_bounds(
        state,
        geometry_vertex_volumes=geometry_lumped,
        reference_vertex_volumes=reference_vertices,
    )
    momentum_enabled = bool(USER_ENABLE_ALE_DDG_VERTEX_MOMENTUM_FLUX)
    if momentum_enabled:
        _gmsh_ensure_ale_vertex_momentum_state(
            state,
            conserved_vertex_volumes=conserved,
            velocities=fluid_u,
        )
        momentum = np.asarray(getattr(state, "ale_vertex_momentum_m3_mps", np.empty((0, 3), dtype=float)), dtype=float)
        if momentum.shape != (len(vertices), 3):
            state.last_ale_vertex_momentum_flux_status = "bad_momentum_state"
            momentum_enabled = False
            momentum = np.zeros((len(vertices), 3), dtype=float)
    else:
        state.last_ale_vertex_momentum_flux_status = "disabled"
        momentum = np.zeros((len(vertices), 3), dtype=float)

    mesh_u = (current_points - reference) / float(dt)
    delta = np.zeros_like(conserved, dtype=float)
    momentum_delta = np.zeros_like(momentum, dtype=float)
    working_volume = np.minimum(
        np.maximum(
            np.nan_to_num(conserved.copy(), nan=0.0, posinf=0.0, neginf=0.0),
            volume_floor,
        ),
        volume_ceiling,
    )
    old_total_momentum = np.sum(momentum, axis=0) if momentum_enabled else np.zeros(3, dtype=float)
    node_id_map = dict(getattr(state, "volume_export_node_ids", {}))
    if not node_id_map:
        node_id_map = {id(vertex): idx for idx, vertex in enumerate(vertices)}
    ...
    try:
        if hasattr(state, "_dual_area_vector_cache"):
            delattr(state, "_dual_area_vector_cache")
        _gmsh_recompute_ddg_duals_for_flux(state)
        cached_pairs = getattr(state, "_ale_ddg_vertex_flux_edge_pairs", None)
        if (
            bool(USER_ALE_DDG_VERTEX_FLUX_CACHE_EDGES)
            and cached_pairs is not None
            and len(cached_pairs) > 0
        ):
            edge_iterable = [(int(i), int(j), vertices[int(i)], vertices[int(j)]) for i, j in cached_pairs]
        else:
            edge_iterable = []
            candidate_pairs: list[tuple[int, int]] = []
            for i, vertex in enumerate(vertices):
                for neighbor in list(getattr(vertex, "nn", [])):
                    j = node_id_map.get(id(neighbor))
                    if j is None or int(j) <= int(i):
                        continue
                    edge_iterable.append((int(i), int(j), vertex, neighbor))
                    candidate_pairs.append((int(i), int(j)))

        for i, j, vertex_i, vertex_j in edge_iterable:
            try:
                area_ij = dual_area_vector(vertex_i, vertex_j, state.HC, dim=3)
            except (KeyError, IndexError, ValueError, RuntimeError, AttributeError, ZeroDivisionError):
                skipped_edges += 1
                continue
            area_ij = np.asarray(area_ij, dtype=float)
            area_norm = float(np.linalg.norm(area_ij))
            if (not np.isfinite(area_norm)) or area_norm <= 1.0e-30:
                skipped_edges += 1
                continue
            u_face = 0.5 * (fluid_u[i] + fluid_u[j])
            w_face = 0.5 * (mesh_u[i] + mesh_u[j])
            volume_flux = float(np.dot(u_face - w_face, area_ij))
            if not np.isfinite(volume_flux):
                skipped_edges += 1
                continue
            raw_transfer = float(dt) * volume_flux
            lower_transfer = max(
                float(working_volume[i] - volume_ceiling[i]),
                float(volume_floor[j] - working_volume[j]),
            )
            upper_transfer = min(
                float(working_volume[i] - volume_floor[i]),
                float(volume_ceiling[j] - working_volume[j]),
            )
            if (
                (not np.isfinite(raw_transfer))
                or (not np.isfinite(lower_transfer))
                or (not np.isfinite(upper_transfer))
                or lower_transfer > upper_transfer
            ):
                transfer = 0.0
                limited_edges += 1
            else:
                transfer = float(np.clip(raw_transfer, lower_transfer, upper_transfer))
                scale = max(
                    abs(raw_transfer),
                    float(volume_floor[i]),
                    float(volume_floor[j]),
                    1.0e-30,
                )
                if abs(transfer - raw_transfer) > max(1.0e-12 * scale, 1.0e-30):
                    limited_edges += 1
            delta[i] -= transfer
            delta[j] += transfer
            working_volume[i] -= transfer
            working_volume[j] += transfer
            if momentum_enabled:
                scheme = str(USER_ALE_DDG_VERTEX_MOMENTUM_FACE_SCHEME).strip().lower()
                if scheme == "upwind":
                    transport_u = fluid_u[i] if volume_flux >= 0.0 else fluid_u[j]
                else:
                    transport_u = u_face
                momentum_transfer = transfer * np.asarray(transport_u, dtype=float)
                if np.all(np.isfinite(momentum_transfer)):
                    momentum_delta[i] -= momentum_transfer
                    momentum_delta[j] += momentum_transfer
            edge_count += 1
        if bool(USER_ALE_DDG_VERTEX_FLUX_CACHE_EDGES) and cached_pairs is None:
            state._ale_ddg_vertex_flux_edge_pairs = tuple(candidate_pairs)
    finally:
        _gmsh_clear_transient_ddg_duals(state)

    state.last_ale_vertex_mass_flux_edges = int(edge_count)
    state.last_ale_vertex_mass_flux_skipped_edges = int(skipped_edges)
    state.last_ale_vertex_mass_flux_limited_edges = int(limited_edges)
    if edge_count <= 0:
        state.last_ale_vertex_mass_flux_status = "no_ddg_edges"
        return

    raw = conserved + delta
    new_values, clipped = _gmsh_renormalize_ale_vertex_volumes(
        state,
        raw,
        lower_bounds=volume_floor,
        upper_bounds=volume_ceiling,
    )
    state.ale_vertex_volumes_m3 = new_values
    if momentum_enabled:
        raw_momentum = momentum + momentum_delta
        raw_total_momentum = np.sum(raw_momentum, axis=0)
        momentum_scale = max(float(np.linalg.norm(old_total_momentum)), float(np.sum(conserved)) * 1.0e-12, 1.0e-30)
        state.last_ale_vertex_momentum_flux_total_rel = float(
            np.linalg.norm(raw_total_momentum - old_total_momentum) / momentum_scale
        )
        rel_momentum_delta = np.linalg.norm(raw_momentum - momentum, axis=1) / np.maximum(
            np.linalg.norm(momentum, axis=1),
            np.maximum(conserved, 1.0e-30) * 1.0e-12,
        )
        state.last_ale_vertex_momentum_flux_max_rel_delta = (
            float(np.max(rel_momentum_delta)) if rel_momentum_delta.size else 0.0
        )
        state.ale_vertex_momentum_m3_mps = np.nan_to_num(
            raw_momentum,
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        )
        velocities_after = state.ale_vertex_momentum_m3_mps / np.maximum(new_values, 1.0e-30)[:, None]
        velocities_after = _gmsh_constrained_velocity_array(
            state,
            velocities_after,
            vertices=vertices,
        )
        state.ale_vertex_momentum_m3_mps = np.maximum(new_values, 1.0e-30)[:, None] * velocities_after

        apply_momentum_velocity = bool(USER_ALE_DDG_MOMENTUM_APPLY_TO_MESH_VELOCITY)
        projected_after_flux = False
        if apply_momentum_velocity:
            current_velocities = np.asarray([np.asarray(v.u[:3], dtype=float) for v in vertices], dtype=float)
            relaxation = float(np.clip(float(USER_ALE_DDG_MOMENTUM_VELOCITY_RELAXATION), 0.0, 1.0))
            if relaxation <= 0.0:
                relaxation = 1.0
            applied_velocities = current_velocities + relaxation * (velocities_after - current_velocities)
            fixed_vertices = set(state.bV_caps)
            for vertex, velocity in zip(vertices, applied_velocities):
                if vertex in fixed_vertices:
                    continue
                vertex.u = np.asarray(velocity, dtype=float)
            _enforce_no_swirl_velocity_field(state)
            _project_gmsh_contact_line_velocities_to_sphere_tangents(state)
            _set_cap_velocities(state)
            if bool(USER_ENABLE_ALE_DDG_POST_MOMENTUM_PROJECTION):
                _apply_gmsh_incompressible_projection(state, dt=dt)
                _enforce_no_swirl_velocity_field(state)
                _project_gmsh_contact_line_velocities_to_sphere_tangents(state)
                _set_cap_velocities(state)
                projected_after_flux = True
            _gmsh_sync_ale_vertex_momentum_from_velocities(state, conserved_vertex_volumes=new_values)
            post_velocities = np.asarray([np.asarray(v.u[:3], dtype=float) for v in vertices], dtype=float)
        else:
            post_velocities = velocities_after
        state.last_ale_vertex_momentum_flux_max_speed = (
            float(np.max(np.linalg.norm(post_velocities, axis=1))) if post_velocities.size else 0.0
        )
        post_total_momentum = np.sum(np.asarray(state.ale_vertex_momentum_m3_mps, dtype=float), axis=0)
        state.last_ale_vertex_momentum_flux_post_constraint_rel = float(
            np.linalg.norm(post_total_momentum - raw_total_momentum) / momentum_scale
        )
        if apply_momentum_velocity:
            state.last_ale_vertex_momentum_flux_status = "applied_projected" if projected_after_flux else "applied"
        else:
            state.last_ale_vertex_momentum_flux_status = "applied_stored"
    target_total = float(getattr(state, "ale_vertex_total_volume_m3", 0.0))
    total = float(np.sum(new_values))
    if np.isfinite(target_total) and target_total > 0.0:
        state.last_ale_vertex_mass_flux_total_rel = float((total - target_total) / max(target_total, 1.0e-30))
    rel_delta = np.abs(new_values - conserved) / np.maximum(reference_vertices, 1.0e-30)
    state.last_ale_vertex_mass_flux_max_rel_delta = float(np.max(rel_delta)) if rel_delta.size else 0.0
    state.last_ale_vertex_mass_flux_min_fraction = float(
        np.min(new_values / np.maximum(reference_vertices, 1.0e-30))
    ) if new_values.size else 1.0
    state.last_ale_vertex_mass_flux_clipped = int(clipped)
    if clipped > 0:
        state.last_ale_vertex_mass_flux_status = "applied_clipped"
    elif limited_edges > 0:
        state.last_ale_vertex_mass_flux_status = "applied_limited"
    elif state.last_ale_vertex_mass_flux_max_rel_delta <= 1.0e-14:
        state.last_ale_vertex_mass_flux_status = "zero_relative_flux"
    else:
        state.last_ale_vertex_mass_flux_status = "applied"
    _axisym_clear_caches(state)
...
def _gmsh_tet_divergence_l2_from_arrays(
    ...
    weighted = 0.0
    volume_sum = 0.0
    if bool(USER_FAST_TET_GEOMETRY_BATCH):
        cell_indices, volumes, grads_by_cell = _tet_volume_and_shape_grads_batch(pts, tet_idx)
        iterable = (
            (tet_idx[int(cell_idx)], float(volume), grads)
            for cell_idx, volume, grads in zip(cell_indices, volumes, grads_by_cell)
        )
    else:
        def _iter_loop():
            for tet in tet_idx:
                ...
                if result is None:
                    continue
                volume, grads = result
                yield idx, volume, grads

        iterable = _iter_loop()
    for idx, volume, grads in iterable:
...
def _apply_gmsh_flux_volume_constraint(
    state: VolumetricPitoisState,
    *,
    dt: float,
    reference_positions: np.ndarray,
) -> None:
    state.last_flux_volume_constraint_status = "not_run"
    state.last_flux_volume_constraint_rel_before = 0.0
    state.last_flux_volume_constraint_rel_after = 0.0
    state.last_flux_volume_constraint_iterations = 0
    state.last_gmsh_cl_volume_slide_status = "not_used_flux"
    state.last_gmsh_cl_volume_slide_scale = 1.0
    state.last_gmsh_cl_volume_slide_rel_before = 0.0
    state.last_gmsh_cl_volume_slide_rel_after = 0.0
    if (
        (not bool(getattr(state, "gmsh_compute_mesh", False)))
        or (not bool(getattr(state.config, "enable_incompressible_projection", False)))
        or (not bool(USER_ENABLE_FLUX_VOLUME_CONSTRAINT))
        or float(dt) <= 0.0
    ):
        state.last_flux_volume_constraint_status = "disabled"
        return
    if coo_matrix is None or diags is None or spsolve is None:
        raise RuntimeError("USER_ENABLE_FLUX_VOLUME_CONSTRAINT requires scipy.sparse.")

    target = float(getattr(state, "target_snapshot_volume_m3", 0.0))
    if (not np.isfinite(target)) or target <= 0.0:
        state.last_flux_volume_constraint_status = "bad_target"
        state.last_position_volume_constraint_status = "flux_bad_target"
        _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
        return
    ...
    tets = np.asarray(getattr(state, "volume_export_tets", np.empty((0, 4), dtype=int)), dtype=int)
    if (not vertices) or tets.size == 0:
        state.last_flux_volume_constraint_status = "empty_mesh"
        state.last_position_volume_constraint_status = "flux_empty_mesh"
        _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
        return
    ...
    if reference.shape != current_points.shape:
        state.last_flux_volume_constraint_status = "bad_reference"
        state.last_position_volume_constraint_status = "flux_bad_reference"
        _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
        return
    if (not np.all(np.isfinite(reference))) or (not np.all(np.isfinite(current_points))):
        state.last_flux_volume_constraint_status = "bad_points"
        state.last_position_volume_constraint_status = "flux_bad_points"
        _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
        return

    current_volume = float(_indexed_tet_mesh_volume_m3(current_points, tets))
    if (not np.isfinite(current_volume)) or current_volume <= 0.0:
        state.last_flux_volume_constraint_status = "bad_current"
        state.last_position_volume_constraint_status = "flux_bad_current"
        _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
        return

    rel_before = float((current_volume - target) / max(target, 1.0e-30))
    state.last_flux_volume_constraint_rel_before = rel_before
    state.last_flux_volume_constraint_rel_after = rel_before
    state.last_position_volume_constraint_rel_before = rel_before
    state.last_position_volume_constraint_rel_after = rel_before
    state.last_gmsh_cl_volume_slide_rel_before = rel_before
    state.last_gmsh_cl_volume_slide_rel_after = rel_before

    tol = max(float(USER_FLUX_VOLUME_CONSTRAINT_REL_TOL), 0.0)
    if abs(rel_before) <= tol:
        state.last_flux_volume_constraint_status = "within_trigger"
        state.last_position_volume_constraint_status = "flux_within_trigger"
        _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
        return
    ...
    if not np.any(movable):
        state.last_flux_volume_constraint_status = "no_movable_vertices"
        state.last_position_volume_constraint_status = "flux_no_movable_vertices"
        _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
        return

    edge_lengths = _gmsh_local_edge_lengths(current_points, tets)
    correction_limits = np.minimum(
        float(USER_FLUX_VOLUME_CONSTRAINT_MAX_CORRECTION_M),
        max(float(USER_FLUX_VOLUME_CONSTRAINT_EDGE_FRACTION), 0.0) * edge_lengths,
    )
    correction_limits = np.maximum(correction_limits, 1.0e-10)
    ...
    def solve_displacement(points: np.ndarray) -> tuple[np.ndarray | None, str]:
        g_rows: list[int] = []
        g_cols: list[int] = []
        g_data: list[float] = []
        ...
        if bool(USER_FAST_TET_GEOMETRY_BATCH):
            cell_indices, volumes, grads_by_cell = _tet_volume_and_shape_grads_batch(points, tets)
            tet_geometry_iterable = (
                (local_col, tets[int(cell_idx)], float(volume), grads)
                for local_col, (cell_idx, volume, grads) in enumerate(
                    zip(cell_indices, volumes, grads_by_cell)
                )
            )
        else:
            def _iter_tet_geometry():
                local_col = 0
                for tet in tets:
                    idx = np.asarray(tet, dtype=int)
                    ...
                    if result is None:
                        continue
                    volume, grads = result
                    yield local_col, idx, volume, grads
                    local_col += 1

            tet_geometry_iterable = _iter_tet_geometry()

        for local_col, idx, volume, grads in tet_geometry_iterable:
            cell_volumes.append(float(volume))
            lumped[idx] += 0.25 * float(volume)
            for a_local, ia in enumerate(idx):
                base = 3 * int(ia)
                for component in range(3):
                    g_rows.append(base + component)
                    g_cols.append(int(local_col))
                    g_data.append(float(volume) * float(grads[a_local, component]))

        n_cells = len(cell_volumes)
        if n_cells == 0:
            return None, "empty_tet_cache"
        volume_array = np.asarray(cell_volumes, dtype=float)
        valid_volume_sum = float(np.sum(volume_array))
        if (not np.isfinite(valid_volume_sum)) or valid_volume_sum <= 0.0:
            return None, "bad_cell_volume_sum"

        w_rows: list[int] = []
        w_cols: list[int] = []
        w_data: list[float] = []
        for idx, vertex in enumerate(vertices):
            if (not movable[idx]) or lumped[idx] <= 1.0e-30:
                continue
            ...
            for a in range(3):
                for b in range(3):
                    ...
                    if abs(value) > 0.0:
                        ...
                        w_data.append(value)
        if not w_data:
            return None, "zero_mobility"

        g_matrix = coo_matrix((g_data, (g_rows, g_cols)), shape=(n_dofs, n_cells)).tocsr()
        mobility = coo_matrix((w_data, (w_rows, w_cols)), shape=(n_dofs, n_dofs)).tocsr()
        desired_volumes = volume_array * (target / max(valid_volume_sum, 1.0e-30))
        rhs = desired_volumes - volume_array
        if not np.all(np.isfinite(rhs)):
            return None, "bad_rhs"
        matrix = (g_matrix.T @ mobility @ g_matrix).tocsr()
        diag_mean = float(np.mean(np.abs(matrix.diagonal()))) if n_cells else 1.0
        diag_mean = max(diag_mean, 1.0e-30)
        damping = max(float(USER_FLUX_VOLUME_CONSTRAINT_REGULARIZATION), 0.0)
        if damping > 0.0:
            matrix = matrix + diags(
                np.full(n_cells, damping * diag_mean, dtype=float),
                0,
        ...
        displacement = np.asarray(mobility @ (g_matrix @ pressure), dtype=float).reshape((n_vertices, 3))
        if not np.all(np.isfinite(displacement)):
            displacement = np.nan_to_num(displacement, nan=0.0, posinf=0.0, neginf=0.0)
        displacement[~movable] = 0.0
        return displacement, "ok"

    working_points = current_points.copy()
    best_points = current_points.copy()
    best_volume = current_volume
    best_rel = rel_before
    accepted_any = False
    failure_status = "no_improvement"

    for iteration in range(max(1, int(USER_FLUX_VOLUME_CONSTRAINT_MAX_ITERS))):
        working_volume = float(_indexed_tet_mesh_volume_m3(working_points, tets))
        if (not np.isfinite(working_volume)) or working_volume <= 0.0:
            failure_status = "bad_iter_volume"
            break
        working_rel = float((working_volume - target) / max(target, 1.0e-30))
        if abs(working_rel) <= tol:
            best_points = working_points.copy()
            best_volume = working_volume
            best_rel = working_rel
            state.last_flux_volume_constraint_iterations = iteration
            break

        displacement, status = solve_displacement(working_points)
        if displacement is None:
            failure_status = status
            break
        step_norm = np.linalg.norm(displacement, axis=1)
        used_norm = np.linalg.norm(working_points - current_points, axis=1)
        active = movable & np.isfinite(step_norm) & (step_norm > 1.0e-30)
        if not np.any(active):
            failure_status = "zero_displacement"
            break
        remaining_active = correction_limits[active] - used_norm[active]
        step_norm_active = step_norm[active]
        usable = np.isfinite(remaining_active) & np.isfinite(step_norm_active) & (remaining_active > 1.0e-30)
        if not np.any(usable):
            failure_status = "correction_limit_reached"
            break
        alpha_cap = min(
            1.0,
            0.98
            * float(
                np.min(
                    remaining_active[usable]
                    / np.maximum(step_norm_active[usable], 1.0e-30)
                )
            ),
        )
        if (not np.isfinite(alpha_cap)) or alpha_cap <= 1.0e-14:
            failure_status = "correction_limit_reached"
            break
        ...
        iteration_best: tuple[float, float, np.ndarray] | None = None
        current_error = abs(working_volume - target)
        for alpha in candidate_alphas:
            if (not np.isfinite(alpha)) or alpha <= 0.0:
                continue
            trial_points = working_points + float(alpha) * displacement
            trial_points[~movable] = current_points[~movable]
            _gmsh_project_trial_points_to_constraints(state, trial_points)
            trial_points[~movable] = current_points[~movable]
            if (
                (not np.all(np.isfinite(trial_points)))
                or float(np.max(np.abs(trial_points))) > max_coord
            ):
                continue
            total_delta_norm = np.linalg.norm(trial_points - current_points, axis=1)
            if np.any(total_delta_norm[movable] > correction_limits[movable]):
                continue
            trial_volume = float(_indexed_tet_mesh_volume_m3(trial_points, tets))
            if (not np.isfinite(trial_volume)) or trial_volume <= 0.0:
                continue
            trial_error = abs(trial_volume - target)
            trial_rel = float((trial_volume - target) / max(target, 1.0e-30))
            if trial_error < current_error:
                if iteration_best is None or trial_error < abs(iteration_best[0] - target):
                    iteration_best = (trial_volume, trial_rel, trial_points)

        if iteration_best is None:
            failure_status = "no_line_search_improvement"
            break

        best_volume, best_rel, best_points = iteration_best
        working_points = best_points.copy()
        accepted_any = True
        state.last_flux_volume_constraint_iterations = iteration + 1
        if abs(best_rel) <= tol:
            break
    ...
    final_rel = float((final_volume - target) / max(target, 1.0e-30))
    state.last_flux_volume_constraint_rel_after = final_rel
    state.last_position_volume_constraint_rel_after = final_rel
    state.last_gmsh_cl_volume_slide_rel_after = final_rel
    _gmsh_set_velocities_from_displacement(state, reference_positions=reference_positions, dt=dt)
    state.incompressible_divergence_after_l2 = float(_gmsh_tet_divergence_l2(state))
    if abs(final_rel) <= tol:
        status = "flux_applied"
    elif abs(final_rel) < abs(rel_before):
        status = "flux_best_effort"
    else:
        status = f"flux_{failure_status}"
    state.last_flux_volume_constraint_status = status
    state.last_position_volume_constraint_status = status
    _axisym_clear_caches(state)
...
def _apply_gmsh_post_edit_continuity_projection(
    ...
    *,
    ...
    if bool(USER_FAST_TET_GEOMETRY_BATCH):
        cell_indices, volumes, grads_by_cell = _tet_volume_and_shape_grads_batch(current_points, tets)
        tet_geometry_iterable = (
            (int(cell_idx), tets[int(cell_idx)], float(volume), grads)
            for cell_idx, volume, grads in zip(cell_indices, volumes, grads_by_cell)
        )
    else:
        def _iter_tet_geometry():
            for cell_idx, tet in enumerate(tets):
                ...
                if result is None:
                    continue
                volume, grads = result
                yield int(cell_idx), idx, volume, grads

        tet_geometry_iterable = _iter_tet_geometry()
    for cell_idx, idx, volume, grads in tet_geometry_iterable:
    ...
    def divergence_l2_from_tet_cache(velocity_array: np.ndarray) -> float:
        weighted = 0.0
        volume_sum = 0.0
    ...
    raw_div_l2 = divergence_l2_from_tet_cache(raw_velocity)
    ...
    def candidate(alpha: float) -> tuple[float, float, float, np.ndarray] | None:
        ...
        if alpha in candidate_cache:
            return candidate_cache[alpha]

        def cache(value: tuple[float, float, float, np.ndarray] | None) -> tuple[float, float, float, np.ndarray] | None:
            candidate_cache[alpha] = value
            return value
        ...
        return cache((volume, rel, div_l2, trial_points))
    ...
    if target > 0.0:
        ...
        if volume_safe_candidates:
            ...
            # Do not trade a growing water-volume error for a lower-divergence
            # correction. Volume conservation is the hard constraint here.
...
def _tet_volume_and_shape_grads_batch(points: np.ndarray, tets: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    pts = np.asarray(points, dtype=float)
    tet_idx = np.asarray(tets, dtype=int)
    if pts.size == 0 or tet_idx.size == 0:
        return np.empty(0, dtype=int), np.empty(0, dtype=float), np.empty((0, 4, 3), dtype=float)

    def slow_path() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        cell_indices: list[int] = []
        volumes: list[float] = []
        grads_by_cell: list[np.ndarray] = []
        for cell_idx, tet in enumerate(tet_idx):
            idx = np.asarray(tet, dtype=int)
            result = _tet_volume_and_shape_grads(pts[idx])
            if result is None:
                continue
            volume, grads = result
            cell_indices.append(int(cell_idx))
            volumes.append(float(volume))
            grads_by_cell.append(np.asarray(grads, dtype=float))
        if not cell_indices:
            return np.empty(0, dtype=int), np.empty(0, dtype=float), np.empty((0, 4, 3), dtype=float)
        return np.asarray(cell_indices, dtype=int), np.asarray(volumes, dtype=float), np.asarray(grads_by_cell, dtype=float)

    try:
        tet_points = pts[tet_idx]
        x = np.ones((int(tet_idx.shape[0]), 4, 4), dtype=float)
        x[:, :, 1:] = tet_points
        det = np.linalg.det(x)
        volumes_all = np.abs(det) / 6.0
        valid = np.isfinite(volumes_all) & (volumes_all > 1.0e-30)
        if not np.any(valid):
            return np.empty(0, dtype=int), np.empty(0, dtype=float), np.empty((0, 4, 3), dtype=float)
        cell_indices = np.flatnonzero(valid)
        inv_x = np.linalg.inv(x[cell_indices])
    except (IndexError, ValueError, np.linalg.LinAlgError, FloatingPointError):
        return slow_path()

    grads_by_cell = np.asarray(np.transpose(inv_x[:, 1:, :], (0, 2, 1)), dtype=float)
    finite_grads = np.all(np.isfinite(grads_by_cell), axis=(1, 2))
    if not np.all(finite_grads):
        cell_indices = cell_indices[finite_grads]
        grads_by_cell = grads_by_cell[finite_grads]
    if cell_indices.size == 0:
        return np.empty(0, dtype=int), np.empty(0, dtype=float), np.empty((0, 4, 3), dtype=float)
    return cell_indices.astype(int, copy=False), np.asarray(volumes_all[cell_indices], dtype=float), grads_by_cell
...
def _advance_gmsh_incompressible_substep(state: VolumetricPitoisState, *, dt: float) -> bool:
    ...
    _apply_gmsh_incompressible_projection(state, dt=dt)
    _set_cap_velocities(state)
    ale_fluid_velocities = np.asarray(
        [np.asarray(v.u[:3], dtype=float) for v in state.volume_export_vertices],
        dtype=float,
    )
    ...
    if bool(USER_ENABLE_FLUX_VOLUME_CONSTRAINT):
        _apply_gmsh_flux_volume_constraint(
            state,
            dt=dt,
            reference_positions=substep_reference_positions,
        )
    if bool(USER_ENABLE_KINEMATIC_VOLUME_CONSTRAINT):
        target = float(getattr(state, "target_snapshot_volume_m3", 0.0))
        current_volume = float(_snapshot_msh_volume_m3(state))
        run_kinematic_fallback = True
        if np.isfinite(target) and target > 0.0 and np.isfinite(current_volume):
            rel_after_flux = float((current_volume - target) / max(target, 1.0e-30))
            run_kinematic_fallback = abs(rel_after_flux) > max(
                float(USER_KINEMATIC_VOLUME_CONSTRAINT_REL_TOL),
                float(USER_FLUX_VOLUME_CONSTRAINT_REL_TOL),
            )
        if run_kinematic_fallback:
            _apply_gmsh_kinematic_volume_constraint(
                state,
                dt=dt,
                reference_positions=substep_reference_positions,
            )
            state.incompressible_divergence_after_l2 = float(_gmsh_tet_divergence_l2(state))
    _assert_fixed_topology(state)
    _apply_gmsh_ale_ddg_vertex_mass_flux(
        state,
        dt=dt,
        reference_positions=substep_reference_positions,
        fluid_velocities=ale_fluid_velocities,
    )
    _update_duals_and_masses(state)
    _update_pressure_scalar(state)
...
def _step_record(state: VolumetricPitoisState, *, step: int, t: float) -> dict[str, float | int]:
    ...
    ale_vertex_conserved_total_m3 = float(
        np.sum(np.asarray(getattr(state, "ale_vertex_volumes_m3", np.empty(0, dtype=float)), dtype=float))
    )
    return {
        "step": int(step),
        "t": float(t),
        ...
        "ale_vertex_conserved_total_m3": ale_vertex_conserved_total_m3,
        "ale_vertex_target_total_m3": float(getattr(state, "ale_vertex_total_volume_m3", 0.0)),
        ...
        "flux_volume_constraint_status": str(getattr(state, "last_flux_volume_constraint_status", "not_run")),
        "flux_volume_constraint_rel_before": float(getattr(state, "last_flux_volume_constraint_rel_before", 0.0)),
        "flux_volume_constraint_rel_after": float(getattr(state, "last_flux_volume_constraint_rel_after", 0.0)),
        "flux_volume_constraint_iterations": int(getattr(state, "last_flux_volume_constraint_iterations", 0)),
        "ale_vertex_mass_flux_status": str(getattr(state, "last_ale_vertex_mass_flux_status", "not_run")),
        "ale_vertex_mass_flux_total_rel": float(getattr(state, "last_ale_vertex_mass_flux_total_rel", 0.0)),
        "ale_vertex_mass_flux_max_rel_delta": float(getattr(state, "last_ale_vertex_mass_flux_max_rel_delta", 0.0)),
        "ale_vertex_mass_flux_min_fraction": float(getattr(state, "last_ale_vertex_mass_flux_min_fraction", 1.0)),
        "ale_vertex_mass_flux_clipped": int(getattr(state, "last_ale_vertex_mass_flux_clipped", 0)),
        "ale_vertex_mass_flux_limited_edges": int(
            getattr(state, "last_ale_vertex_mass_flux_limited_edges", 0)
        ),
        "ale_vertex_mass_flux_edges": int(getattr(state, "last_ale_vertex_mass_flux_edges", 0)),
        "ale_vertex_mass_flux_skipped_edges": int(getattr(state, "last_ale_vertex_mass_flux_skipped_edges", 0)),
        "ale_vertex_momentum_flux_status": str(getattr(state, "last_ale_vertex_momentum_flux_status", "not_run")),
        "ale_vertex_momentum_flux_total_rel": float(getattr(state, "last_ale_vertex_momentum_flux_total_rel", 0.0)),
        "ale_vertex_momentum_flux_post_constraint_rel": float(
            getattr(state, "last_ale_vertex_momentum_flux_post_constraint_rel", 0.0)
        ),
        "ale_vertex_momentum_flux_max_rel_delta": float(
            getattr(state, "last_ale_vertex_momentum_flux_max_rel_delta", 0.0)
        ),
        "ale_vertex_momentum_flux_max_speed": float(getattr(state, "last_ale_vertex_momentum_flux_max_speed", 0.0)),
...
_HISTORY_ARRAY_CACHE: dict[int, SimpleNamespace] = {}


def _history_arrays_incremental(history: list[dict]) -> dict[str, np.ndarray]:
    key = id(history)
    cached = _HISTORY_ARRAY_CACHE.get(key)
    n_rows = len(history)
    if cached is None or cached.history is not history or cached.length > n_rows:
        arrays = _history_arrays(history)
        _HISTORY_ARRAY_CACHE[key] = SimpleNamespace(history=history, length=n_rows, arrays=arrays)
        return arrays
    if cached.length == n_rows:
        return cached.arrays

    new_rows = history[int(cached.length) :]
    if not new_rows:
        return cached.arrays
    d_over_r = np.array([float(row["d_over_r"]) for row in new_rows], dtype=float)
    force_mn = np.array([float(row["fixed_force_axial"]) for row in new_rows], dtype=float) * 1.0e3
    t_ms = np.array([float(row["t"]) for row in new_rows], dtype=float) * 1.0e3
    max_u = np.array([float(row["max_free_speed"]) for row in new_rows], dtype=float)
    arrays = {
        "d_over_r": np.concatenate((cached.arrays["d_over_r"], d_over_r)),
        "force_mn": np.concatenate((cached.arrays["force_mn"], force_mn)),
        "force_abs_mn": np.concatenate((cached.arrays["force_abs_mn"], np.abs(force_mn))),
        "t_ms": np.concatenate((cached.arrays["t_ms"], t_ms)),
        "max_u": np.concatenate((cached.arrays["max_u"], max_u)),
    }
    cached.length = n_rows
    cached.arrays = arrays
    return arrays
...
_FIG5_COMPARE_RENDERER_CACHE: dict[tuple, SimpleNamespace] = {}


def _fig5_compare_renderer(
    *,
    exp_x: np.ndarray,
    exp_y: np.ndarray,
    out_path: Path,
    ...
    exp_x_arr = np.asarray(exp_x, dtype=float)
    exp_y_arr = np.asarray(exp_y, dtype=float)
    key = (
        str(Path(out_path).resolve()),
        tuple(float(v) for v in xlim),
        tuple(float(v) for v in ylim),
        str(title),
        tuple(float(v) for v in exp_x_arr),
        tuple(float(v) for v in exp_y_arr),
    )
    cached = _FIG5_COMPARE_RENDERER_CACHE.get(key)
    if cached is not None:
        return cached

    fig_kwargs = {"figsize": (7.4, 5.6)}
    if bool(USER_FAST_FIG5_REFRESH):
        fig_kwargs["dpi"] = 200
    fig, ax = plt.subplots(**fig_kwargs)
    ax.scatter(exp_x_arr, exp_y_arr, s=34, color="#111111", label="Pitois 2000 dynamic exp (black dots)")
    (sim_line,) = ax.plot([], [], color="#d62828", linewidth=2.0, label="Case_2b_axisym simulation")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlim(*xlim)
    ax.set_xlabel(r"$D/R$")
    ...
    fig.tight_layout()
    if bool(USER_FAST_FIG5_REFRESH) and Image is not None:
        sim_line.set_animated(True)
    cached = SimpleNamespace(fig=fig, ax=ax, sim_line=sim_line, background=None, background_ylim=None)
    _FIG5_COMPARE_RENDERER_CACHE[key] = cached
    return cached
...
def _save_pitois_fig5_compare(
    *,
    ...
    renderer = _fig5_compare_renderer(
        exp_x=exp_x,
        exp_y=exp_y,
        out_path=out_path,
        xlim=xlim,
    ...
    fig = renderer.fig
    ax = renderer.ax
    sim_x_arr = np.asarray(sim_x, dtype=float)
    sim_y_arr = np.asarray(sim_y, dtype=float)
    y_low, y_high = _fig5_visible_ylim(
        exp_x=exp_x,
        exp_y=exp_y,
        sim_x=sim_x_arr,
        sim_y=sim_y_arr,
    ...
    renderer.sim_line.set_data(sim_x_arr, sim_y_arr)
    ax.set_ylim(y_low, y_high)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    if bool(USER_FAST_FIG5_REFRESH) and Image is not None:
        y_lim = (float(y_low), float(y_high))
        if renderer.background is None or renderer.background_ylim != y_lim:
            fig.canvas.draw()
            renderer.background = fig.canvas.copy_from_bbox(fig.bbox)
            renderer.background_ylim = y_lim
        else:
            fig.canvas.restore_region(renderer.background)
        ax.draw_artist(renderer.sim_line)
        fig.canvas.blit(fig.bbox)
        width, height = fig.canvas.get_width_height()
        image = Image.frombuffer("RGBA", (width, height), fig.canvas.buffer_rgba(), "raw", "RGBA", 0, 1)
        image.save(out_path)
    elif bool(USER_FAST_FIG5_REFRESH):
        fig.canvas.draw()
...
def _refresh_pitois_fig5_compare_png(
    *,
    ...
    if not separation_history:
        return
    separation = _history_arrays_incremental(separation_history)
    exp_x, exp_y = _pitois_fig5_dynamic_digitized()
    compare_mask = np.array(
        [int(row.get("step", 0)) >= int(USER_FIG5_COMPARE_SKIP_INITIAL_STEPS) for row in separation_history],
        dtype=bool,
...
def _print_header(config: VolumetricPitoisConfig) -> None:
    ...
    if bool(getattr(config, "gmsh_compute_mesh", False)) or bool(
        ...
        print(
            "Gmsh flux volume fix      = "
            f"{'on' if USER_ENABLE_FLUX_VOLUME_CONSTRAINT else 'off'}"
            f" (tol {USER_FLUX_VOLUME_CONSTRAINT_REL_TOL:.3e}, "
            f"max correction {USER_FLUX_VOLUME_CONSTRAINT_MAX_CORRECTION_M:.3e} m)"
        )
        print(
            "Gmsh ALE DDG vertex flux  = "
            f"{'on' if USER_ENABLE_ALE_DDG_VERTEX_MASS_FLUX else 'off'}"
            f" (min vertex fraction {USER_ALE_VERTEX_MASS_MIN_FRACTION:.3e}, "
            f"geom floor {USER_ALE_VERTEX_MASS_GEOMETRY_FLOOR_FRACTION:.3e}, "
            f"ref floor {USER_ALE_VERTEX_MASS_REFERENCE_FLOOR_FRACTION:.3e}, "
            f"batch duals {'on' if USER_ALE_DDG_VERTEX_FLUX_BATCH_DUALS else 'off'}, "
            f"edge cache {'on' if USER_ALE_DDG_VERTEX_FLUX_CACHE_EDGES else 'off'})"
        )
        print(
            "Gmsh ALE DDG momentum     = "
            f"{'on' if USER_ENABLE_ALE_DDG_VERTEX_MOMENTUM_FLUX else 'off'}"
            f" (face {USER_ALE_DDG_VERTEX_MOMENTUM_FACE_SCHEME}, "
            f"mesh velocity {'on' if USER_ALE_DDG_MOMENTUM_APPLY_TO_MESH_VELOCITY else 'off'}, "
            f"post projection {'on' if USER_ENABLE_ALE_DDG_POST_MOMENTUM_PROJECTION else 'off'})"
        )
    print(
...
def run_motion_case(
    ...
    *,
    ...
    for step in range(config.n_steps):
        ...
        record_row = None
        verbose_every = max(1, int(USER_VERBOSE_EVERY_STEPS))
        should_print_step = bool(verbose) and (
            completed_step == 1
            or completed_step == int(config.n_steps)
            or completed_step % verbose_every == 0
        )
        if should_print_step or completed_step % max(1, config.record_every) == 0:
        ...
        if should_print_step:
            ...
            if record_row is not None:
                ...
                print(
                    "Flux volume fix          = "
                    f"{record_row['flux_volume_constraint_status']}, "
                    f"iters {int(record_row['flux_volume_constraint_iterations'])}, "
                    f"rel {float(record_row['flux_volume_constraint_rel_before']):+.6e}"
                    f" -> {float(record_row['flux_volume_constraint_rel_after']):+.6e}",
                    flush=True,
                )
                print(
                    "ALE DDG vertex flux      = "
                    f"{record_row['ale_vertex_mass_flux_status']}, "
                    f"total rel {float(record_row['ale_vertex_mass_flux_total_rel']):+.6e}, "
                    f"max vertex dV/V0 {float(record_row['ale_vertex_mass_flux_max_rel_delta']):.6e}, "
                    f"min vertex frac {float(record_row['ale_vertex_mass_flux_min_fraction']):.6e}, "
                    f"edges {int(record_row['ale_vertex_mass_flux_edges'])}, "
                    f"skipped {int(record_row['ale_vertex_mass_flux_skipped_edges'])}, "
                    f"limited {int(record_row['ale_vertex_mass_flux_limited_edges'])}, "
                    f"clipped {int(record_row['ale_vertex_mass_flux_clipped'])}",
                    flush=True,
                )
                print(
                    "ALE DDG momentum flux    = "
                    f"{record_row['ale_vertex_momentum_flux_status']}, "
                    f"raw total rel {float(record_row['ale_vertex_momentum_flux_total_rel']):.6e}, "
                    f"constraint rel {float(record_row['ale_vertex_momentum_flux_post_constraint_rel']):.6e}, "
                    f"max dP/P0 {float(record_row['ale_vertex_momentum_flux_max_rel_delta']):.6e}, "
                    f"max speed {float(record_row['ale_vertex_momentum_flux_max_speed']):.6e} m/s",
                    flush=True,
                )
                if bool(getattr(state.config, "enable_incompressible_projection", False)):
                    print(
