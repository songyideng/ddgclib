...
def _gmsh_renormalize_ale_vertex_volumes(
    ...
    clipped = int(np.count_nonzero(values < floor))
    values = np.maximum(np.nan_to_num(values, nan=0.0, posinf=0.0, neginf=0.0), floor)
    ...
    if (not np.isfinite(current_total)) or current_total <= 0.0:
        values = np.maximum(reference.copy(), floor)
        current_total = float(np.sum(values))
    if abs(current_total - target_total) <= max(1.0e-14 * target_total, 1.0e-30):
        return values, clipped
    if current_total > target_total:
        surplus = values - floor
        surplus_sum = float(np.sum(surplus))
        if surplus_sum > 1.0e-30:
            values = floor + surplus * max((target_total - float(np.sum(floor))) / surplus_sum, 0.0)
    else:
        weights = np.maximum(reference, 1.0e-30)
        values += (target_total - current_total) * weights / max(float(np.sum(weights)), 1.0e-30)
    return values, clipped
...
def _apply_gmsh_ale_ddg_vertex_mass_flux(
    ...
    *,
    ...
    try:
        ...
        for i, j, vertex_i, vertex_j in edge_iterable:
            ...
            if not np.isfinite(volume_flux):
                skipped_edges += 1
                continue
            delta[i] -= float(dt) * volume_flux
            delta[j] += float(dt) * volume_flux
            edge_count += 1
    ...
    reference_vertices = np.asarray(getattr(state, "ale_vertex_reference_volumes_m3", np.empty(0, dtype=float)), dtype=float)
    if reference_vertices.size != conserved.size:
        reference_vertices = np.maximum(conserved.copy(), 1.0e-30)
    raw = conserved + delta
    new_values, clipped = _gmsh_renormalize_ale_vertex_volumes(state, raw)
