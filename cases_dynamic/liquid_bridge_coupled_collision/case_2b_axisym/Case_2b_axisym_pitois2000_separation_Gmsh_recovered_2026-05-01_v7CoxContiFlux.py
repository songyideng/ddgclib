...
# Refreshing the live Fig. 5 PNG every integration step is pure output cost.
# Keep the force history at every step for the final F plot, but only rewrite
# the live PNG periodically.
USER_FIG5_LIVE_REFRESH_EVERY_STEPS = 1
...
# Guard the Gmsh compute mesh as a valid tetrahedral map, not just as a sum of
# absolute cell volumes. A cell that changes orientation has folded through
# zero volume, and a cell that becomes too small makes projection/pressure
# operators ill-conditioned even if the global absolute volume still matches.
USER_ENABLE_TET_QUALITY_GUARD = True
USER_TET_QUALITY_MIN_VOLUME_FRACTION = 1.0e-3
USER_TET_QUALITY_ABSOLUTE_MIN_VOLUME_M3 = 1.0e-20
...
class VolumetricPitoisState:
    ...
    reference_tet_signs: np.ndarray = field(default_factory=lambda: np.empty(0, dtype=float))
    reference_tet_volumes_m3: np.ndarray = field(default_factory=lambda: np.empty(0, dtype=float))
    last_tet_quality_status: str = "not_run"
    last_tet_quality_flipped: int = 0
    last_tet_quality_collapsed: int = 0
    last_tet_quality_min_volume_fraction: float = 1.0
    last_tet_quality_min_volume_m3: float = 0.0
...
def _tet_signed_volumes_m3(points: np.ndarray, tets: np.ndarray) -> np.ndarray:
    ...
    return np.asarray(triple, dtype=float) / 6.0


def _gmsh_tet_quality_report_from_signed(
    state: VolumetricPitoisState,
    signed: np.ndarray,
) -> dict[str, float | int | bool | str]:
    signed = np.asarray(signed, dtype=float)
    reference_signs = np.asarray(getattr(state, "reference_tet_signs", np.empty(0, dtype=float)), dtype=float)
    reference_volumes = np.asarray(getattr(state, "reference_tet_volumes_m3", np.empty(0, dtype=float)), dtype=float)
    if signed.size == 0 or reference_signs.size != signed.size or reference_volumes.size != signed.size:
        return {
            "ok": False,
    ...
    if (not np.all(np.isfinite(signed))) or (not np.all(np.isfinite(reference_signs))) or (not np.all(np.isfinite(reference_volumes))):
        return {
            "ok": False,
            "status": "nonfinite_tet_volume",
            "flipped": 0,
            "collapsed": 0,
            "min_volume_fraction": 0.0,
            "min_volume_m3": 0.0,
    ...
    abs_floor = max(float(USER_TET_QUALITY_ABSOLUTE_MIN_VOLUME_M3), 0.0)
    reference_floor = np.maximum(reference_volumes, max(abs_floor, 1.0e-300))
    min_allowed = np.maximum(abs_floor, max(float(USER_TET_QUALITY_MIN_VOLUME_FRACTION), 0.0) * reference_floor)
    flipped = int(np.count_nonzero(signed * reference_signs <= 0.0))
    collapsed = int(np.count_nonzero(abs_volume < min_allowed))
    min_volume = float(np.min(abs_volume)) if abs_volume.size else 0.0
    volume_fraction = abs_volume / reference_floor
    min_fraction = float(np.min(volume_fraction)) if volume_fraction.size else 0.0

    if flipped > 0 and collapsed > 0:
        status = "flipped_and_collapsed"
    elif flipped > 0:
        status = "flipped"
    elif collapsed > 0:
        status = "collapsed"
    ...
    return {
        ...
        "flipped": flipped,
        "collapsed": collapsed,
        "min_volume_fraction": min_fraction,
        "min_volume_m3": min_volume,
    }


def _gmsh_tet_quality_report(state: VolumetricPitoisState, points: np.ndarray) -> dict[str, float | int | bool | str]:
    if (not bool(USER_ENABLE_TET_QUALITY_GUARD)) or (not bool(getattr(state, "gmsh_compute_mesh", False))):
        return {
            "ok": True,
            "status": "disabled",
            "flipped": 0,
    ...
    tets = np.asarray(getattr(state, "volume_export_tets", np.empty((0, 4), dtype=int)), dtype=int)
    signed = _tet_signed_volumes_m3(points, tets)
    return _gmsh_tet_quality_report_from_signed(state, signed)


def _record_tet_quality_report(state: VolumetricPitoisState, report: dict[str, float | int | bool | str]) -> None:
    state.last_tet_quality_status = str(report.get("status", "unknown"))
    state.last_tet_quality_flipped = int(report.get("flipped", 0))
    state.last_tet_quality_collapsed = int(report.get("collapsed", 0))
    state.last_tet_quality_min_volume_fraction = float(report.get("min_volume_fraction", 0.0))
    state.last_tet_quality_min_volume_m3 = float(report.get("min_volume_m3", 0.0))


def _gmsh_tet_quality_ok(
    state: VolumetricPitoisState,
    points: np.ndarray,
    *,
    update_state: bool = False,
) -> bool:
    report = _gmsh_tet_quality_report(state, points)
    if update_state:
        _record_tet_quality_report(state, report)
    return bool(report.get("ok", False))


def _assert_gmsh_tet_quality_points(
    state: VolumetricPitoisState,
    points: np.ndarray,
    *,
    where: str,
    ...
    if bool(report.get("ok", False)):
        return
    message = (
        f"Tetrahedral mesh quality failed after {where}: {report.get('status')}; "
        f"flipped={int(report.get('flipped', 0))}, "
        f"collapsed={int(report.get('collapsed', 0))}, "
        f"min_volume_fraction={float(report.get('min_volume_fraction', 0.0)):.6e}, "
        f"min_volume={float(report.get('min_volume_m3', 0.0)):.6e} m^3"
    )
...
def _assert_gmsh_tet_quality(state: VolumetricPitoisState, *, where: str) -> None:
    _assert_gmsh_tet_quality_points(state, _volume_points_array(state), where=where)
...
def _state_finite_summary(state: VolumetricPitoisState) -> tuple[bool, str]:
    ...
    gmsh_signed_volumes = None
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        tets = np.asarray(getattr(state, "volume_export_tets", np.empty((0, 4), dtype=int)), dtype=int)
        gmsh_signed_volumes = _tet_signed_volumes_m3(coords, tets)
        volume = float(np.sum(np.abs(gmsh_signed_volumes))) if gmsh_signed_volumes.size else 0.0
    else:
    ...
    if bool(getattr(state, "gmsh_compute_mesh", False)) and bool(USER_ENABLE_TET_QUALITY_GUARD):
        tet_quality = _gmsh_tet_quality_report_from_signed(state, gmsh_signed_volumes)
        _record_tet_quality_report(state, tet_quality)
        if not bool(tet_quality.get("ok", False)):
            return False, (
                f"tet quality {tet_quality.get('status')}: "
                f"flipped={int(tet_quality.get('flipped', 0))}, "
                f"collapsed={int(tet_quality.get('collapsed', 0))}, "
                f"min_volume_fraction={float(tet_quality.get('min_volume_fraction', 0.0)):.6e}, "
                f"min_volume={float(tet_quality.get('min_volume_m3', 0.0)):.6e} m^3"
            )
...
def _prepare_gmsh_state(config: VolumetricPitoisConfig) -> VolumetricPitoisState:
    ...
    initial_signed_volumes = _tet_signed_volumes_m3(points, state.volume_export_tets)
    state.reference_tet_signs = np.sign(initial_signed_volumes)
    state.reference_tet_volumes_m3 = np.abs(initial_signed_volumes)
    _record_tet_quality_report(state, _gmsh_tet_quality_report(state, points))
...
def _update_duals_and_masses(state: VolumetricPitoisState) -> None:
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        ...
        lumped = np.zeros(len(vertices), dtype=float)
        if tets.size:
            share = 0.25 * np.abs(_tet_signed_volumes_m3(points, tets))
            for local_idx in range(4):
                np.add.at(lumped, tets[:, local_idx], share)
        for vertex, mass in zip(vertices, lumped):
            vertex.m = max(float(mass), 1.0e-12)
        _axisym_clear_caches(state)
        return
    ...
    *,
    ...
    def trial_points_at(scale: float) -> np.ndarray | None:
        ...
        if not np.all(np.isfinite(trial_points)):
            return None
        if not _gmsh_tet_quality_ok(state, trial_points):
            return None
        return trial_points
    ...
    _assert_gmsh_tet_quality(state, where="kinematic volume constraint")
    final_volume = float(_snapshot_msh_volume_m3(state))
...
def _apply_gmsh_flux_volume_constraint(
    ...
    *,
    ...
    if accepted_any:
        ...
        _assert_gmsh_tet_quality(state, where="flux volume constraint")
        final_volume = float(_snapshot_msh_volume_m3(state))
    else:
...
def _apply_gmsh_post_edit_continuity_projection(
    ...
    *,
    ...
    _assert_gmsh_tet_quality(state, where="post-edit continuity projection")
    final_volume = float(_snapshot_msh_volume_m3(state))
...
def _advance_gmsh_incompressible_substep(state: VolumetricPitoisState, *, dt: float) -> bool:
    ...
    if moving_vertices:
        trial_points = _volume_points_array(state).copy()
        node_id_map = dict(getattr(state, "volume_export_node_ids", {}))
        for vertex, target in zip(moving_vertices, targets):
            idx = node_id_map.get(id(vertex))
            if idx is not None:
                trial_points[int(idx)] = np.asarray(target, dtype=float)
        _assert_gmsh_tet_quality_points(state, trial_points, where="free-vertex velocity update")
    ...
    _assert_gmsh_tet_quality(state, where="Gmsh incompressible substep")
    _assert_fixed_topology(state)
    return True
...
def _step_record(state: VolumetricPitoisState, *, step: int, t: float) -> dict[str, float | int]:
    ...
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        points = _volume_points_array(state)
        tets = np.asarray(getattr(state, "volume_export_tets", np.empty((0, 4), dtype=int)), dtype=int)
        snapshot_msh_volume_m3 = _indexed_tet_mesh_volume_m3(points, tets)
        dual_cell_sum_m3 = snapshot_msh_volume_m3
    else:
        dual_cell_sum_m3 = _mesh_volume_m3(state.HC)
        snapshot_msh_volume_m3 = _snapshot_msh_volume_m3(state)
    vertices_for_count = list(state.HC.V)
    n_vertices = len(vertices_for_count)
    n_free_vertices = sum(1 for v in vertices_for_count if v not in state.bV_caps)
    return {
        "step": int(step),
        "t": float(t),
        "gap": gap,
        ...
        "tet_quality_status": str(getattr(state, "last_tet_quality_status", "not_run")),
        "tet_quality_flipped": int(getattr(state, "last_tet_quality_flipped", 0)),
        "tet_quality_collapsed": int(getattr(state, "last_tet_quality_collapsed", 0)),
        "tet_quality_min_volume_fraction": float(getattr(state, "last_tet_quality_min_volume_fraction", 1.0)),
        "tet_quality_min_volume_m3": float(getattr(state, "last_tet_quality_min_volume_m3", 0.0)),
        "max_free_speed": _max_free_speed(state),
        "pressure_scalar": float(state.pressure_scalar),
        "n_vertices": int(n_vertices),
        "n_free_vertices": int(n_free_vertices),
    }
...
def _should_refresh_live_fig5(completed_step: int, config: VolumetricPitoisConfig) -> bool:
    refresh_every = int(USER_FIG5_LIVE_REFRESH_EVERY_STEPS)
    if refresh_every <= 0:
        return False
    completed_step = int(completed_step)
    return completed_step == int(config.n_steps) or completed_step % max(1, refresh_every) == 0
...
def _print_header(config: VolumetricPitoisConfig) -> None:
    ...
    print(
        "Live Fig. 5 refresh       = "
        f"every {USER_FIG5_LIVE_REFRESH_EVERY_STEPS} step(s); final F plot keeps every recorded force row"
    )
    if config.dt > 0.0 and not config.enable_adaptive_dt:
...
def run_motion_case(
    ...
    *,
    ...
    for step in range(config.n_steps):
        ...
        if save_fig and _should_refresh_live_fig5(completed_step, config):
            _refresh_pitois_fig5_compare_png(
                separation_history=history,
                out_dir=out_dir,
            )
