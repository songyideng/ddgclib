...
Important limitation:
    This case does not solve a full volumetric pressure field. Pressure is kept
    as an internal-flow pressure field only; liquid-gas curvature is applied
    separately through the Heron surface-tension operator, and the contact-line
    ring uses a dynamic-contact-angle force.
...
OUT_ROOT = Path(__file__).resolve().parent / "out" / "Case_2b_axisym_pitois2000_separation_Gmsh"
...
class VolumetricPitoisConfig:
    ...
    # Pressure is reserved for the internal flow field; interface curvature is
    # handled separately through the Heron surface-tension force.
...
def _update_pressure_scalar(state: VolumetricPitoisState) -> None:
    # Flow pressure only.  Do not put Young-Laplace/interface curvature into
    # p_i; curvature enters through _surface_tension_force_heron().
    state.pressure_scalar = float(getattr(state.config, "internal_pressure_pa", 0.0))
...
def _Fcl(v, *, state: VolumetricPitoisState) -> np.ndarray:
    ...
    for ring, sphere_center, sphere_velocity, which in specs:
        ...
        return (
            ...
            * line_length
            * (float(np.cos(theta_dyn)) - float(np.cos(theta_eq)))
            * slide_dir
        )
    return np.zeros(3, dtype=float)
...
def _Ftot(
    ...
    *,
    ...
    # Ftot = Fp,flow + Fmu + Fs,Heron + Fp,proj + Fcl
...
def _print_header(config: VolumetricPitoisConfig) -> None:
    ...
    print("Contact-line force        = dynamic contact angle")
