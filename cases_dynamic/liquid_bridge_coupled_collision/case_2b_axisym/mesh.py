...
USER_INTERACTIVE_ELEV_DEG = 0#25.0  # side-on reference: 0
USER_INTERACTIVE_AZIM_DEG = 90#45.0  # side-on reference: -87
USER_LIQUID_BRIDGE_ALPHA = 1
USER_SPHERE_ALPHA = 1
USER_SPHERE_VISIBLE_FRACTION = 1.0 / 5.0
USER_VERTEX_SIZE = 5.0

DEFAULT_PARTICLE_RADIUS_M = 4.0e-3
LIQUID_FACE_COLOR = "#8fb0c5"
LIQUID_EDGE_COLOR = "#324f62"
...
def _set_axes_equal(ax, coords: np.ndarray, *, pad_fraction: float = 0.06) -> None:
...
def _plot_surface(
    ...
    *,
    ...
    _set_axes_equal(ax, np.vstack(axis_points))
    ax.set_xlabel("x [mm]")
    ax.set_ylabel("y [mm]")
    ax.set_zlabel("z [mm]")
    ...
    fig.subplots_adjust(left=0.03, right=0.98, bottom=0.04, top=0.84)

    slider_ax = fig.add_axes([0.22, 0.92, 0.56, 0.03])
    vertex_slider = Slider(
        slider_ax,
        "vertex size",
        valmin=0.0,
        valmax=max(30.0, 4.0 * vertex_size),
        valinit=vertex_size,
        valstep=0.5,
    )

    def _update_vertex_size(value: float) -> None:
        vertex_scatter.set_sizes(np.full(surface_vertices.shape, float(value), dtype=float))
        fig.canvas.draw_idle()

    vertex_slider.on_changed(_update_vertex_size)
    fig._case2b_widgets = [vertex_slider]
    return fig
