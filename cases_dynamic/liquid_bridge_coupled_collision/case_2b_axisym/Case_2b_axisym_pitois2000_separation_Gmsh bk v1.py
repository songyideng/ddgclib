...
USER_MESH_SNAPSHOT_EVERY_STEPS = 0
...
USER_VOLUME_PROJECTION_MAX_ITERS = 30
USER_VOLUME_PROJECTION_REL_TOL = 1.0e-7
USER_MAX_ACCELERATION = 5.0e-3
USER_ABORT_ON_NONFINITE_STATE = True
USER_ENABLE_GMSH_GEOMETRIC_VOLUME_CORRECTION = False
USER_GMSH_GEOMETRIC_VOLUME_CORRECTION_TRIGGER_REL = 1.0e-4
...
class VolumetricPitoisState:
    ...
    surface_export_boundary_tris: np.ndarray = field(default_factory=lambda: np.empty((0, 3), dtype=int))
...
def _gmsh_surface_heron_force_map(state: VolumetricPitoisState) -> dict[int, np.ndarray]:
    ...
    if side_tris.size == 0:
        ...
        if len(ordered_rings) < 2:
            state._gmsh_surface_heron_force_cache = {}
            return state._gmsh_surface_heron_force_cache
        flat_sources = [v for ring in ordered_rings for v in ring]
        edges, _triangles, _boundary_indices = _structured_surface_topology(ordered_rings)
        surface_clone = Complex(3, domain=None)
        clone_by_idx = {
            idx: surface_clone.V[tuple(float(x) for x in np.asarray(source_v.x_a[:3], dtype=float))]
            for idx, source_v in enumerate(flat_sources)
        }
        for a, b in edges:
            clone_by_idx[int(a)].connect(clone_by_idx[int(b)])
    else:
        used_indices = sorted({int(idx) for tri in side_tris for idx in tri})
        flat_sources = surface_vertices
        surface_clone = Complex(3, domain=None)
        clone_by_idx = {
    ...
    force_map = {
        id(flat_sources[idx]): surface_tension_force(surface_v, gamma=state.config.gamma, dim=3)
        for idx, surface_v in clone_by_idx.items()
    }
...
def _triangle_vertex_indices(triangles: np.ndarray) -> set[int]:
    tri_idx = np.asarray(triangles, dtype=int)
    if tri_idx.size == 0:
        return set()
    return {int(idx) for tri in tri_idx for idx in tri}


def _gmsh_side_rings_from_current_boundary(
    state: VolumetricPitoisState,
    side_indices: set[int],
) -> list[list[object]]:
    if not side_indices:
        return [list(ring) for ring in state.outer_rings if ring]
    node_ids = dict(getattr(state, "surface_export_node_ids", {}))
    rings: list[list[object]] = []
    for spec in getattr(state, "gmsh_axisym_ring_specs", []):
        ring = [
            v
            for v in list(getattr(spec, "vertices", []))
            if int(node_ids.get(id(v), -1)) in side_indices
        ]
        if len(ring) >= 3:
            rings.append(ring)
    if len(rings) < 2:
    ...
    rings.sort(
        key=lambda ring: float(
            np.mean([float(np.dot(np.asarray(v.x_a[:3], dtype=float) - mid, axis)) for v in ring])
        )
    )
    return rings


def _refresh_gmsh_dynamic_boundaries(
    state: VolumetricPitoisState,
    *,
    project_contact_to_spheres: bool = False,
) -> None:
    ...
    if not vertices:
        return
    boundary_tris = np.asarray(
        getattr(state, "surface_export_boundary_tris", np.empty((0, 3), dtype=int)),
        dtype=int,
    )
    if boundary_tris.size == 0:
        boundary_tris = np.concatenate(
            [
                np.asarray(getattr(state, "surface_export_side_tris", np.empty((0, 3), dtype=int)), dtype=int),
                np.asarray(getattr(state, "surface_export_bottom_cap_tris", np.empty((0, 3), dtype=int)), dtype=int),
                np.asarray(getattr(state, "surface_export_top_cap_tris", np.empty((0, 3), dtype=int)), dtype=int),
            ],
            axis=0,
        )
    node_ids = dict(getattr(state, "surface_export_node_ids", {}))
    if boundary_tris.size == 0:
        side_tris = np.empty((0, 3), dtype=int)
        bottom_cap_tris = np.empty((0, 3), dtype=int)
        top_cap_tris = np.empty((0, 3), dtype=int)
        side_rings = [list(ring) for ring in state.outer_rings if ring]
        if len(side_rings) < 2:
            return
        side_indices = {
            int(node_ids[id(v)])
        ...
        bottom_cap_indices = {int(node_ids[id(v)]) for v in state.cap_bottom if id(v) in node_ids}
        top_cap_indices = {int(node_ids[id(v)]) for v in state.cap_top if id(v) in node_ids}
    else:
        points = np.asarray([np.asarray(v.x_a[:3], dtype=float) for v in vertices], dtype=float)
        ...
        side_indices = _triangle_vertex_indices(side_tris)
        bottom_cap_indices = _triangle_vertex_indices(bottom_cap_tris)
        top_cap_indices = _triangle_vertex_indices(top_cap_tris)
        if not side_indices:
            return
        side_rings = _gmsh_side_rings_from_current_boundary(state, side_indices)
    # The physical contact line is the current liquid-air side endpoint, not a
    # permanent initial cap/side triangle intersection.
    bottom_contact_ring = list(side_rings[0]) if side_rings else []
    top_contact_ring = list(side_rings[-1]) if side_rings else []
    bottom_contact_indices = {int(node_ids[id(v)]) for v in bottom_contact_ring if id(v) in node_ids}
    top_contact_indices = {int(node_ids[id(v)]) for v in top_contact_ring if id(v) in node_ids}
    bottom_cap_indices |= bottom_contact_indices
    top_cap_indices |= top_contact_indices
    ...
    state.outer_rings = side_rings
    state.layer_rings = [[list(ring)] for ring in side_rings]
    state.layer_centers = [
        min(ring, key=lambda v: float(np.linalg.norm(np.asarray(v.x_a[:2], dtype=float))))
        for ring in side_rings
    ]
    if len(side_rings) >= 2:
        z_vals = np.asarray([np.mean([float(v.x_a[2]) for v in ring]) for ring in side_rings], dtype=float)
        z_span = max(float(z_vals[-1] - z_vals[0]), 1.0e-30)
        state.layer_fractions = tuple(float((z - z_vals[0]) / z_span) for z in z_vals)

    state.bottom_contact_ring = bottom_contact_ring
    state.top_contact_ring = top_contact_ring
    state.cap_bottom = [vertices[int(i)] for i in sorted(bottom_cap_indices)]
    state.cap_top = [vertices[int(i)] for i in sorted(top_cap_indices)]
    bottom_contact_ids = {id(v) for v in state.bottom_contact_ring}
    top_contact_ids = {id(v) for v in state.top_contact_ring}
    state.cap_bottom_interior = [v for v in state.cap_bottom if id(v) not in bottom_contact_ids]
    state.cap_top_interior = [v for v in state.cap_top if id(v) not in top_contact_ids]
    state.bV_caps = set(state.cap_bottom_interior + state.cap_top_interior)
    if state.cap_bottom:
        state.cap_bottom_center = min(
            state.cap_bottom,
            key=lambda v: float(np.linalg.norm(np.asarray(v.x_a[:2], dtype=float))),
        )
    if state.cap_top:
        state.cap_top_center = min(
            state.cap_top,
            key=lambda v: float(np.linalg.norm(np.asarray(v.x_a[:2], dtype=float))),
        )

    for idx, v in enumerate(vertices):
        in_bottom = int(idx) in bottom_cap_indices
        in_top = int(idx) in top_cap_indices
        in_side = int(idx) in side_indices
        v.cap_id = "bottom" if in_bottom else ("top" if in_top else None)
        v.boundary = bool(in_side or in_bottom or in_top)
        v.is_interface = bool(in_side)
        v.interface_phases = frozenset({0, 1}) if in_side else frozenset()

    if project_contact_to_spheres:
...
def _prepare_gmsh_state(config: VolumetricPitoisConfig) -> VolumetricPitoisState:
    ...
    state.surface_export_boundary_tris = np.asarray(triangles, dtype=int)
    state.surface_export_side_tris = np.asarray(side_tris, dtype=int)
    state.surface_export_bottom_cap_tris = np.asarray(bottom_cap_tris, dtype=int)
    state.surface_export_top_cap_tris = np.asarray(top_cap_tris, dtype=int)
    _refresh_gmsh_dynamic_boundaries(state, project_contact_to_spheres=True)
    state.last_bottom_contact_line_radius = float(_cap_radius(state.bottom_contact_ring)) if state.bottom_contact_ring else 0.0
    state.last_top_contact_line_radius = float(_cap_radius(state.top_contact_ring)) if state.top_contact_ring else 0.0
    state.frozen_volume_vertex_id_order = tuple(id(vertex) for vertex in state.volume_export_vertices)
    state.frozen_surface_vertex_id_order = tuple(id(vertex) for vertex in state.surface_export_vertices)
    state.frozen_layer_ring_id_structure = _layer_ring_id_structure(state)
    state.topology_frozen = True
...
def _axisym_force_cache_key(
    ...
    *,
    ...
    p_proj_key = (
        ...
        if bool(include_projected_pressure)
        else 0.0
    )
    return (
...
def _gmsh_axisymmetrize_state(state: VolumetricPitoisState) -> None:
    ...
    _refresh_gmsh_dynamic_boundaries(state, project_contact_to_spheres=False)
    _set_cap_velocities(state)
    _axisym_clear_caches(state)
...
def _update_gmsh_contact_line_speed_from_current_radius(
    state: VolumetricPitoisState,
    *,
    dt: float,
) -> None:
    if not bool(getattr(state, "gmsh_compute_mesh", False)):
        return
    _refresh_gmsh_dynamic_boundaries(state, project_contact_to_spheres=False)
    current_bottom_radius = float(_cap_radius(state.bottom_contact_ring)) if state.bottom_contact_ring else 0.0
    current_top_radius = float(_cap_radius(state.top_contact_ring)) if state.top_contact_ring else 0.0
    prev_bottom_radius = float(getattr(state, "last_bottom_contact_line_radius", current_bottom_radius))
    prev_top_radius = float(getattr(state, "last_top_contact_line_radius", current_top_radius))
    sphere_radius = max(float(state.config.particle_radius), 1.0e-30)
    if float(dt) > 0.0:
        alpha_bottom_prev = float(np.arcsin(np.clip(prev_bottom_radius / sphere_radius, -1.0, 1.0)))
        alpha_bottom_new = float(np.arcsin(np.clip(current_bottom_radius / sphere_radius, -1.0, 1.0)))
        alpha_top_prev = float(np.arcsin(np.clip(prev_top_radius / sphere_radius, -1.0, 1.0)))
        alpha_top_new = float(np.arcsin(np.clip(current_top_radius / sphere_radius, -1.0, 1.0)))
        state.last_bottom_contact_line_speed = abs(
            sphere_radius * (alpha_bottom_new - alpha_bottom_prev) / float(dt)
        )
        state.last_top_contact_line_speed = abs(
            sphere_radius * (alpha_top_new - alpha_top_prev) / float(dt)
        )
    else:
        state.last_bottom_contact_line_speed = 0.0
        state.last_top_contact_line_speed = 0.0
    state.last_bottom_contact_line_radius = current_bottom_radius
    state.last_top_contact_line_radius = current_top_radius
...
def _step_record(state: VolumetricPitoisState, *, step: int, t: float) -> dict[str, float | int]:
    ...
    top_cl_radius = float(
        ...
        else _cap_radius(state.outer_rings[-1])
    )
    ...
    return {
        ...
        "bottom_contact_radius": float(
            _cap_radius(state.bottom_contact_ring)
            if state.bottom_contact_ring
            else _cap_radius(state.outer_rings[0])
        ),
        "top_contact_radius": float(top_cl_radius),
...
def _indexed_tet_mesh_volume_m3(points: np.ndarray, tets: np.ndarray) -> float:
    ...
    signed_six_vol = np.einsum("ij,ij->i", pa - pd, np.cross(pb - pd, pc - pd))
    return float(np.sum(np.abs(signed_six_vol)) / 6.0)
...
def _output_csv_row(row: dict) -> dict:
    ...
    aliases = {
        ...
        "dt_s": "dt",
        "gap_m": "gap",
        "top_cl_avg_ucl_x_mps": "top_cl_avg_ux_mps",
        "top_cl_avg_ucl_y_mps": "top_cl_avg_uy_mps",
        "top_cl_avg_ucl_z_mps": "top_cl_avg_uz_mps",
    }
    values = {}
    for field in _OUTPUT_CSV_FIELDS:
        if field == "experiment_first_dF_blackwhite_mN":
            values[field] = float(PITOIS_FIG5_FIRST_DF_BLACKWHITE_SIGNED_MN)
        elif field in {"top_sphere_Ftot_mN", "top_sphere_force_mN"}:
            values[field] = float(row.get(field, 1.0e3 * top_sphere_force_n))
        elif field in {"top_sphere_Fp_flow_lub_mN", "top_sphere_Fp_flow_raw_plus_lub_mN"}:
            values[field] = float(row.get(field, row.get("top_sphere_Fp_flow_mN", 0.0)))
        elif field == "step":
            values[field] = int(row.get("step", 0))
        elif field in {
            "diagnostic_lv_H_avg_vertex_count",
            "p0_neck_fit_vertex_count",
            "p0_neck_fit_half_window_rings",
            ...
            values[field] = int(row.get(aliases.get(field, field), 0 if field != "p0_neck_ring_index" else -1))
        else:
            values[field] = float(row.get(aliases.get(field, field), 0.0))
    return values
...
def _save_output_csv(history: list[dict], out_dir: Path) -> Path:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=_OUTPUT_CSV_FIELDS)
        writer.writeheader()
        for row in history:
...
def _append_output_csv_row(row: dict, out_dir: Path, *, reset: bool = False) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "output.csv"
    write_header = bool(reset) or not path.exists() or path.stat().st_size == 0
    ...
    force_mn = np.array(
        [
            float(
                row.get(
                    "Ftopspherecap_p0_mN",
                    row.get("top_sphere_Fp0_mN", float(row["fixed_force_axial"]) * 1.0e3),
                )
            )
            for row in history
        ],
...
def _advance_state(state: VolumetricPitoisState, *, n_steps: int) -> None:
    ...
    for _step in range(int(n_steps)):
        ...
        for _substep in range(substeps):
            ...
            _update_gmsh_contact_line_speed_from_current_radius(state, dt=sub_dt)
            _assert_fixed_topology(state)
...
def _write_pitois_fig5_comparison(
    *,
    ...
    _save_pitois_fig5_compare(
        ...
        xlim=(0.01, 0.30),
        ylim=(0.02, 2.0),
        dpi=120 if refresh_only else 200,
    )
    if refresh_only:
        return
    ...
    summary = {
        "separation": {
            ...
            "fig5_compare_note": (
                ...
                "post-advance computed dynamics only. The plotted simulation force is "
                "|Ftopspherecap_p0_mN| = |p0*pi*r_neck^2|; contact-line force diagnostics "
                "are written separately and are not added to this Fig. 5 comparison."
            ),
        },
    ...
    *,
    ...
    for step in range(config.n_steps):
        ...
        if completed_step % max(1, config.record_every) == 0:
            ...
            if save_results:
                _append_output_csv_row(history[-1], out_dir, reset=(len(history) == 1))
            if save_fig:
