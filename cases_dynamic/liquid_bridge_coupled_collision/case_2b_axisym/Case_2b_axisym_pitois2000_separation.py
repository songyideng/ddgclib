...
OUT_ROOT = Path(__file__).resolve().parent / "out" / "Case_2b_axisym"
...
def _load_last_initialshape_profile_into_state(state: VolumetricPitoisState) -> Path | None:
    ...
    final_mesh = INITIALSHAPE_OUT_ROOT / "fig" / "mesh_initial.msh"
    if final_mesh.exists() and final_mesh != msh_path:
        final_points, final_tets = _read_gmsh2_tet_mesh(final_mesh)
        if final_points.shape[0] == len(volume_vertices):
            _move_vertices_batch(volume_vertices, [tuple(point) for point in final_points], state.HC, state.bV_caps)
            _refresh_cap_boundary_sets(state)
            _update_duals_and_masses(state)
            _update_pressure_scalar(state)
            if final_tets.size:
                state.target_snapshot_volume_m3 = float(_indexed_tet_mesh_volume_m3(final_points, final_tets))
            return final_mesh

    raise RuntimeError(
        "Initialshape .msh topology does not match the current Case_2b_axisym mesh controls. "
        "Rerun Case_2b_axisym_initialshape.py after changing neck/radial/axial mesh settings."
    )
