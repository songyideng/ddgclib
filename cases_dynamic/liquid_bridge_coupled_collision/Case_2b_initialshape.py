...
def _enforce_interface_radial_distribution(state: sep.VolumetricPitoisState) -> None:
    template = sep._initial_axisymmetric_template(state)
    ...
    contact_radius = float(state.config.target_cap_radius)
    neck_radius = _interface_outer_radius(layer_data[center_idx])
...
def _match_initial_hyperct_msh_volume(state: sep.VolumetricPitoisState) -> float:
    ...
    _enforce_interface_radial_distribution(state)
    sep._update_duals_and_masses(state)
    sep._update_pressure_scalar(state)
...
def _ring_segments(state: sep.VolumetricPitoisState) -> np.ndarray:
    ...
    rings = [list(ring) for ring in state.outer_rings[first_liquid_layer : last_liquid_layer + 1]]
    for ring in rings:
        for i, vertex in enumerate(ring):
            segments.append([_vertex_xyz(vertex), _vertex_xyz(ring[(i + 1) % len(ring)])])
    for lower, upper in zip(rings[:-1], rings[1:]):
        n_ring = min(len(lower), len(upper))
        for i in range(n_ring):
            segments.append([_vertex_xyz(lower[i]), _vertex_xyz(upper[i])])
...
def _render_interface_only_snapshot(state: sep.VolumetricPitoisState, title: str, path: Path) -> Path:
    ...
    if rings:
        pts = np.asarray([_vertex_xyz(vertex) for ring in rings for vertex in ring], dtype=float)
        ax.scatter(1.0e3 * pts[:, 0], 1.0e3 * pts[:, 1], 1.0e3 * pts[:, 2], s=5.0, c="#111111", depthshade=False)
        for ring in (rings[0], rings[-1]):
            cl = np.asarray([_vertex_xyz(vertex) for vertex in ring], dtype=float)
