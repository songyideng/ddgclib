...
USER_ENABLE_MESH_QUALITY_GUARD = True
...
class VolumetricPitoisState:
    ...
    continuity_area_map: dict[int, float] = field(default_factory=dict)
    continuity_normal_map: dict[int, np.ndarray] = field(default_factory=dict)
...
def _refresh_layer_fractions_from_current_geometry(state: VolumetricPitoisState) -> None:
    ...
    for ring in state.outer_rings:
        center = np.mean([np.asarray(v.x_a[:3], dtype=float) for v in ring], axis=0)
        fractions.append(float(np.clip(np.dot(center - bottom_ring_center, axis) / total_span, 0.0, 1.0)))
    state.layer_fractions = tuple(fractions)


def _refresh_cap_ring_factors_from_current_geometry(state: VolumetricPitoisState) -> None:
    if not state.layer_rings or not state.layer_rings[0]:
        return
    outer_radius = max(_cap_radius(state.layer_rings[0][-1]), 1.0e-30)
    state.cap_ring_factors = tuple(
        max(_cap_radius(ring), 0.0) / outer_radius
        for ring in state.layer_rings[0]
    )
...
def _freeze_volume_msh_topology(state: VolumetricPitoisState) -> None:
    ...
    if np.asarray(state.volume_export_tets, dtype=int).size:
        return
    ...
    for pair_idx, (lower_center, upper_center, lower_radial_rings, upper_radial_rings) in enumerate(layer_pairs):
...
def _move_vertex_safe(vertex, target, HC, bV) -> None:
    target = tuple(map(float, target))
    try:
        _move(vertex, target, HC, bV)
    except KeyError:
        stale_keys = [key for key, value in HC.V.cache.items() if value is vertex]
        for key in stale_keys:
            HC.V.cache.pop(key, None)
        HC.V.cache[vertex.x] = vertex
        _move(vertex, target, HC, bV)
...
def _move_vertices_batch(vertices, targets, HC, bV) -> None:
    ...
    if len(vertices) == 1:
        _move_vertex_safe(vertices[0], targets[0], HC, bV)
        return
    ...
    for v, tmp in zip(vertices, staged):
        _move_vertex_safe(v, tmp, HC, bV)
    for v, target in zip(vertices, targets):
        _move_vertex_safe(v, target, HC, bV)
...
def _F_continuity_pressure(v, *, state: VolumetricPitoisState) -> np.ndarray:
    p_continuity = float(getattr(state, "continuity_pressure_scalar", 0.0))
    if abs(p_continuity) <= 1.0e-30:
        return np.zeros(3, dtype=float)
    area = float(state.continuity_area_map.get(id(v), 0.0))
    normal = state.continuity_normal_map.get(id(v))
    if area <= 0.0 or normal is None:
        return np.zeros(3, dtype=float)
    return p_continuity * area * np.asarray(normal, dtype=float)


def _update_continuity_pressure_scalar(state: VolumetricPitoisState, *, dt: float) -> None:
    state.continuity_pressure_scalar = 0.0
    state.continuity_area_map = {}
    state.continuity_normal_map = {}
    if (not state.config.enable_continuity_pressure) or dt <= 0.0:
        return

    if np.asarray(state.volume_export_tets, dtype=int).size:
        gradients = _volume_gradient_array(state)
        ...
        normal_map: dict[int, np.ndarray] = {}
        for vertex, grad in zip(state.volume_export_vertices, gradients):
            grad = np.asarray(grad, dtype=float)
            if float(np.linalg.norm(grad)) <= 1.0e-30:
                continue

            if vertex in state.bV_caps:
                u_pred = np.asarray(vertex.u[:3], dtype=float)
            else:
                Ftot = _Ftot(
                    vertex,
                    state=state,
                    pressure_model=pressure_model,
                    include_continuity_pressure_force=False,
                    include_contact_line=True,
                    include_damping=True,
                )
                accel = Ftot / max(float(getattr(vertex, "m", 0.0)), 1.0e-12)
                accel = _clip_acceleration(accel, state)
                u_pred = np.asarray(vertex.u[:3], dtype=float) + float(dt) * accel

                direction = grad
                if axis_origin is not None and axis is not None:
                    direction = _remove_swirl_component(
                        direction,
                ...
                contribution = float(np.dot(grad, direction)) / max(float(getattr(vertex, "m", 0.0)), 1.0e-12)
                if contribution > 1.0e-30:
                    C += contribution
                    area_map[id(vertex)] = 1.0
                    normal_map[id(vertex)] = direction

            dVdt_pred += float(np.dot(grad, u_pred))

        if C > 1.0e-30:
            state.continuity_pressure_scalar = float(-dVdt_pred / (float(dt) * C))
            state.continuity_area_map = area_map
            state.continuity_normal_map = normal_map
            return
...
def _volume_gradient_array(state: VolumetricPitoisState) -> np.ndarray:
    _ensure_volume_export_node_ids(state)
    ...
    if points.ndim != 2 or points.shape[1] != 3 or tets.ndim != 2 or tets.shape[1] != 4 or tets.size == 0:
        return gradients
    ...
    c = tet_pts[:, 2, :]
    d = tet_pts[:, 3, :]
    grad_a = np.cross(b - d, c - d) / 6.0
    grad_b = np.cross(c - d, a - d) / 6.0
    grad_c = np.cross(a - d, b - d) / 6.0
    grad_d = -(grad_a + grad_b + grad_c)
    for local_idx, grad in enumerate((grad_a, grad_b, grad_c, grad_d)):
        np.add.at(gradients, tets[:, local_idx], grad)
    return gradients


def _enforce_tet_volume_velocity_constraint(state: VolumetricPitoisState) -> bool:
    gradients = _volume_gradient_array(state)
    if gradients.size == 0 or not state.volume_export_vertices:
        return False
    ...
    for vertex, grad in zip(state.volume_export_vertices, gradients):
        ...
        if float(np.linalg.norm(grad)) <= 1.0e-30:
            continue
        flux += float(np.dot(grad, np.asarray(vertex.u[:3], dtype=float)))
        if vertex in state.bV_caps:
            continue
        direction = grad
        if axis_origin is not None and axis is not None:
        ...
        direction = np.asarray(direction, dtype=float)
        contribution = float(np.dot(grad, direction))
        if contribution <= 1.0e-30:
            continue
        denom += contribution
        entries.append((vertex, direction))

    if denom <= 1.0e-30:
        return False
    lambda_u = -flux / denom
    for vertex, direction in entries:
        vertex.u = np.asarray(vertex.u[:3], dtype=float) + lambda_u * direction
    return True
...
def _enforce_continuity_velocity_constraint(state: VolumetricPitoisState) -> None:
    if not bool(getattr(state.config, "enable_continuity_pressure", False)):
        return
    if np.asarray(state.volume_export_tets, dtype=int).size and _enforce_tet_volume_velocity_constraint(state):
        return
    boundary_vertices, area_map, normal_map = _boundary_dual_areas_and_normals(state)
    if not boundary_vertices:
        return
...
def _enforce_min_adjacent_layer_spacing(state: VolumetricPitoisState) -> None:
    ...
    bottom_center, top_center, axis = _contact_plane_centers_and_axis(SimpleNamespace(outer_rings=state.outer_rings))
    del bottom_center, top_center
    for _ in range(3):
        changed = False
...
def _regularize_layer_order(state: VolumetricPitoisState) -> None:
    ...
    for k in range(1, len(state.outer_rings) - 1):
        ...
        for ring in state.layer_rings[k]:
            for v in ring:
                _move_vertex_safe(v, tuple(np.asarray(v.x_a[:3], dtype=float) + shift), state.HC, state.bV_caps)
...
def _apply_initial_unduloid_like_profile(
    ...
    *,
    ...
    for layer, outer_radius in zip(layer_data, outer_radii):
        ...
        _move_vertex_safe(layer["center_vertex"], tuple(center_coord), state.HC, state.bV_caps)
...
def _msh_tet_diagnostics(points: np.ndarray, tets: np.ndarray) -> dict[str, float | int]:
    signed = _tet_signed_volumes(points, tets)
    if signed.size == 0:
        return {
            "volume_m3": 0.0,
            "negative_tets": 0,
            "zero_or_negative_tets": 0,
            "min_signed_m3": 0.0,
        }
    return {
        "volume_m3": float(np.sum(np.abs(signed))),
        "negative_tets": int(np.count_nonzero(signed < 0.0)),
        "zero_or_negative_tets": int(np.count_nonzero(signed <= 0.0)),
        "min_signed_m3": float(np.min(signed)),
    }


def _iter_number_from_msh_path(path: Path) -> int:
    match = re.fullmatch(r"mesh_iter(\d+)", path.stem)
    return int(match.group(1)) if match else -1


def _valid_initialshape_msh_path(
    *,
    expected_nodes: int,
    target_volume_m3: float,
    rel_tol: float = 1.0e-5,
) -> Path | None:
    target = abs(float(target_volume_m3))
    if target <= 0.0:
        return None

    candidate_groups: list[list[Path]] = []

    separation_fig_dir = OUT_ROOT / "fig" / "separation"
    if separation_fig_dir.exists():
        iter_paths = sorted(
            separation_fig_dir.glob("mesh_iter*.msh"),
            key=_iter_number_from_msh_path,
        )
        initial_mesh = separation_fig_dir / "mesh_initial.msh"
        candidate_groups.append(([initial_mesh] if initial_mesh.exists() else []) + iter_paths)

    initialshape_fig_dir = INITIALSHAPE_OUT_ROOT / "fig"
    if initialshape_fig_dir.exists():
        iter_paths = sorted(
            initialshape_fig_dir.glob("mesh_iter*.msh"),
            key=_iter_number_from_msh_path,
            reverse=True,
        )
        final_mesh = initialshape_fig_dir / "mesh_initial.msh"
        candidate_groups.append(iter_paths + ([final_mesh] if final_mesh.exists() else []))

    for paths in candidate_groups:
        for path in paths:
            try:
                points, tets = _read_gmsh2_tet_mesh(path)
            except Exception:
                continue
            if points.shape != (int(expected_nodes), 3) or np.asarray(tets, dtype=int).size == 0:
                continue
            diag = _msh_tet_diagnostics(points, tets)
            if int(diag["zero_or_negative_tets"]) > 0:
                continue
            rel_err = abs(float(diag["volume_m3"]) - target) / max(target, 1.0e-30)
...
def _load_last_initialshape_profile_into_state(state: VolumetricPitoisState) -> Path | None:
    volume_vertices = list(state.HC.V)
    msh_path = _valid_initialshape_msh_path(
        expected_nodes=len(volume_vertices),
    ...
    diag = _msh_tet_diagnostics(all_points, tets)
    ...
    state.imported_volume_msh_path = str(msh_path)
    state.target_volume_m3 = float(diag["volume_m3"])
    state.target_snapshot_volume_m3 = float(diag["volume_m3"])
    _refresh_cap_boundary_sets(state)
    _refresh_layer_fractions_from_current_geometry(state)
    _refresh_cap_ring_factors_from_current_geometry(state)
    return msh_path
...
def _apply_axisymmetric_profile_direct(
    ...
    *,
    ...
    for layer_idx, (center_vertex, rings, axial_s, outer_radius) in enumerate(
        zip(state.layer_centers, state.layer_rings, target_s, target_r)
    ):
        center_coord = origin + float(axial_s) * axis
        _move_vertex_safe(center_vertex, tuple(center_coord), state.HC, state.bV_caps)
...
def _structured_state_from_profile(
    ...
    *,
    ...
    _update_continuity_pressure_scalar(state_new, dt=max(float(config.dt), 1.0e-12))
...
def _cap_vertices_for_side(state: VolumetricPitoisState, *, which: str) -> list:
...
def _cap_boundary_is_effectively_flat(
    state: VolumetricPitoisState,
    *,
    which: str,
    axis: np.ndarray,
) -> bool:
    vertices = _cap_vertices_for_side(state, which=which)
    if len(vertices) < 3:
        return False
    ...
    axis = np.asarray(axis, dtype=float)
    axis /= max(float(np.linalg.norm(axis)), 1.0e-30)
    axial_span = float(np.ptp(coords @ axis))

    contact_ring = state.outer_rings[0] if which == "bottom" else state.outer_rings[-1]
    contact_radius = float(_cap_radius(contact_ring))
    sphere_radius = max(float(state.config.particle_radius), 1.0e-30)
    spherical_sag = sphere_radius - float(np.sqrt(max(sphere_radius**2 - contact_radius**2, 0.0)))
    flat_tolerance = max(1.0e-8, 0.10 * spherical_sag)
    return axial_span <= flat_tolerance


def _rebuild_flat_cap_on_plane(
    state: VolumetricPitoisState,
    *,
    which: str,
    contact_radius: float,
    ...
    e1, e2 = _orthonormal_tangent_basis(axis, reference)
    plane_center = np.asarray(center_vertex.x_a[:3], dtype=float)
    cap_velocity = (
    ...
    center_vertex.u = cap_velocity.copy() if dt is not None and dt > 0.0 else np.zeros(3, dtype=float)
    for factor, ring in zip(state.cap_ring_factors, cap_rings):
        ring_radius = max(float(factor) * float(contact_radius), 0.0)
        targets = []
        for i, _v in enumerate(ring):
            phi = 2.0 * np.pi * float(i) / float(len(ring))
            target = plane_center + ring_radius * (np.cos(phi) * e1 + np.sin(phi) * e2)
            targets.append(tuple(target))
        for v, target in zip(ring, targets):
            old = np.asarray(v.x_a[:3], dtype=float)
            _move_vertex_safe(v, target, state.HC, state.bV_caps)
            if dt is not None and dt > 0.0:
                correction_velocity = (np.asarray(target, dtype=float) - old) / float(dt)
                v.u = correction_velocity if float(np.linalg.norm(correction_velocity)) > 1.0e-30 else cap_velocity.copy()
            else:
                v.u = np.zeros(3, dtype=float)
    _refresh_cap_boundary_sets(state)
...
def _rebuild_cap_on_sphere(
    ...
    *,
    ...
    _move_vertex_safe(center_vertex, tuple(new_center), state.HC, state.bV_caps)
    if dt is not None and dt > 0.0:
...
def _update_moving_contact_line(
    ...
    *,
    ...
    bottom_flat = _cap_boundary_is_effectively_flat(state, which="bottom", axis=axis)
    top_flat = _cap_boundary_is_effectively_flat(state, which="top", axis=axis)
    bottom_radius = prev_bottom_radius if bottom_flat else _estimate_contact_radius(
        state,
        which="bottom",
        sphere_center=state.bottom_sphere_center,
    ...
    top_radius = prev_top_radius if top_flat else _estimate_contact_radius(
        state,
        which="top",
        sphere_center=state.top_sphere_center,
    ...
    if not getattr(state, "imported_volume_msh_path", ""):
...
def _capture_dynamic_state(state: VolumetricPitoisState) -> dict[str, object]:
    ...
    return {
        ...
        "continuity_pressure_scalar": float(getattr(state, "continuity_pressure_scalar", 0.0)),
        "continuity_area_map": dict(getattr(state, "continuity_area_map", {})),
        "continuity_normal_map": {
            int(key): np.asarray(value, dtype=float).copy()
            for key, value in getattr(state, "continuity_normal_map", {}).items()
        },
...
def _restore_dynamic_state(state: VolumetricPitoisState, snapshot: dict[str, object]) -> None:
    ...
    state.continuity_pressure_scalar = float(snapshot["continuity_pressure_scalar"])
    state.continuity_area_map = dict(snapshot["continuity_area_map"])
    state.continuity_normal_map = {
        int(key): np.asarray(value, dtype=float).copy()
        for key, value in snapshot["continuity_normal_map"].items()
    }
...
def _advance_one_substep(state: VolumetricPitoisState, *, dt: float) -> None:
    ...
    _update_duals_and_masses(state)
    _update_pressure_scalar(state)
    _update_continuity_pressure_scalar(state, dt=dt)
    symplectic_euler(
        state.HC,
        state.bV_caps,
        _vertex_acceleration,
...
def _advance_substep_with_quality_guard(
    ...
    *,
    ...
    if depth >= max_retries or float(dt) <= min_dt:
        failure_parts = []
        if bool(metrics.get("orientation_fail", False)):
            failure_parts.append(
                f"negative_tets={int(metrics.get('negative_tets', 0.0))}, "
                f"signed_volume_min={metrics['signed_volume_min_m3']:.3e} m^3"
            )
...
def _prepare_state(config: VolumetricPitoisConfig) -> VolumetricPitoisState:
    ...
    state.target_snapshot_volume_m3 = float(state.target_volume_m3)
    _initialize_mesh_quality_baseline(state)
    if loaded_msh_path is not None:
        print(f"Imported .msh volume      = {1.0e9 * state.target_snapshot_volume_m3:.6f} uL")
    return state
...
def _move_caps(state: VolumetricPitoisState, *, dt: float | None = None) -> None:
    ...
    if float(np.linalg.norm(bottom_disp)) > 1.0e-30:
        for v in state.cap_bottom_interior:
            ...
            _move_vertex_safe(v, tuple(target), state.HC, state.bV_caps)
            v.u = bottom_vel
...
def _Ftot(
    ...
    *,
    state: VolumetricPitoisState,
    pressure_model=None,
    include_continuity_pressure_force: bool = True,
    ...
    # Ftot = F_sigma,p,mu + F_continuity + Fcl - c u
    ...
    if include_continuity_pressure_force:
        Ftot += _F_continuity_pressure(v, state=state)
...
def _vertex_acceleration(v, *, state: VolumetricPitoisState) -> np.ndarray:
    ...
    Ftot = _Ftot(v, state=state, include_continuity_pressure_force=False)
    accel = Ftot / mass
    accel = _clip_acceleration(accel, state)
    accel += _F_continuity_pressure(v, state=state) / mass
    if bool(getattr(state.config, "enforce_no_swirl", False)):
        axis_origin, axis = _swirl_axis_geometry(state)
    ...
    return np.sum(
        [
            _Ftot(
                v,
                state=state,
                include_continuity_pressure_force=True,
...
def _cap_traction_force(
    ...
    *,
    ...
    for a, b, c in tri_idx:
        ...
        for vtx in (va, vb, vc):
            p_v = float(pressure_model(vtx)) + p_continuity
...
def _mesh_quality_is_acceptable(state: VolumetricPitoisState) -> tuple[bool, dict[str, float]]:
    ...
    orientation_fail = (
        float(metrics.get("negative_tets", 0.0)) > 0.0
        or float(metrics.get("signed_volume_min_m3", 0.0)) <= 0.0
    )
    fail_quality = (
        not np.isfinite(metrics["quality_min"])
    ...
    metrics.update(
        {
            ...
            "orientation_fail": bool(orientation_fail),
    ...
    ok = not (orientation_fail or enforce_quality or enforce_edge or enforce_spacing)
...
def _print_header(config: VolumetricPitoisConfig) -> None:
    ...
    print(f"Continuity pressure       = {'on' if config.enable_continuity_pressure else 'off'}")
