...
def _Fp_proj(v, *, state: VolumetricPitoisState) -> np.ndarray:
    p_proj = float(getattr(state, "continuity_pressure_scalar", 0.0))
...
def _prescribed_volume_boundary_vertex_ids(state: VolumetricPitoisState) -> set[int]:
    ...
    ids.update(id(v) for v in state.bottom_contact_ring)
...
def _load_last_initialshape_profile_into_state(state: VolumetricPitoisState) -> Path | None:
    ...
    imported_volume_m3 = abs(float(state.config.initial_bridge_volume_m3))
    ...
    if all_points.shape == (len(volume_vertices), 3):
        ...
        _refresh_cap_boundary_sets(state)
        state.target_snapshot_volume_m3 = float(imported_volume_m3)
        return msh_path

    print(
        "Initial .msh topology mismatch; using axisymmetric profile fallback "
        f"({all_points.shape[0]} nodes in .msh, {len(volume_vertices)} in current mesh). "
        "Rerun Case_2b_initialshape.py for exact volumetric import.",
        flush=True,
    )
    ...
    matched_volume_m3 = _match_positive_msh_volume(
        state,
        target_volume_m3=imported_volume_m3,
    )
    state.target_snapshot_volume_m3 = float(abs(imported_volume_m3))
    if matched_volume_m3 <= 0.0:
        state.target_snapshot_volume_m3 = abs(float(state.config.initial_bridge_volume_m3))
    return msh_path
...
def _radially_scaled_positions(
    ...
    *,
    ...
    if not np.all(np.isfinite(positions)):
        return np.nan_to_num(positions, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if len(getattr(state, "outer_rings", [])) >= 2:
        bottom_ring = np.array([np.asarray(v.x_a[:3], dtype=float) for v in state.outer_rings[0]], dtype=float)
        top_ring = np.array([np.asarray(v.x_a[:3], dtype=float) for v in state.outer_rings[-1]], dtype=float)
        s_min = float(np.mean((bottom_ring - mid[None, :]) @ axis))
        s_max = float(np.mean((top_ring - mid[None, :]) @ axis))
    else:
        s_min = float(np.min(axial_mag))
        s_max = float(np.max(axial_mag))

    span = max(s_max - s_min, 1.0e-30)
    frac = np.clip((axial_mag - s_min) / span, 0.0, 1.0)
    cl_taper = np.sin(np.pi * frac) ** 2
    local_scale = 1.0 + (float(radial_scale) - 1.0) * cl_taper
    scaled = mid[None, :] + axial + local_scale[:, None] * radial
...
def _match_positive_msh_volume(
    state: VolumetricPitoisState,
    *,
    target_volume_m3: float,
    ...
    if not np.isfinite(current) or current <= 0.0:
        return current
    if abs(current - target) / max(target, 1.0e-30) <= rel_tol:
        return current
    ...
    def apply_scale(scale: float) -> float:
        trial_positions = _radially_scaled_positions(
            state,
            base_positions,
    ...
    if current < target:
        ...
        while vol_high < target and high < 16.0:
            high *= 1.10
            vol_high = apply_scale(high)
        if vol_high < target:
            return float(_snapshot_msh_volume_m3(state))
    ...
    best_scale = high if current < target else low
    for _ in range(max(1, int(max_iters))):
        mid = 0.5 * (low + high)
        vol_mid = apply_scale(mid)
        best_scale = mid
        if abs(vol_mid - target) / max(target, 1.0e-30) <= rel_tol:
            break
        if vol_mid < target:
            low = mid
        else:
    ...
    final_volume = apply_scale(best_scale)
    _update_duals_and_masses(state)
...
def _prepare_state(config: VolumetricPitoisConfig) -> VolumetricPitoisState:
    ...
    if imported_target_msh_volume_m3 is not None:
        ...
        _match_positive_msh_volume(
            state,
            target_volume_m3=state.target_snapshot_volume_m3,
        )
