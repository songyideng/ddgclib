...
def _capture_dynamic_state(state: VolumetricPitoisState) -> dict[str, object]:
    ...
    return {
        ...
        "pressure_projection_scalar": float(getattr(state, "pressure_projection_scalar", 0.0)),
        "pressure_projection_area_map": dict(getattr(state, "pressure_projection_area_map", {})),
        "pressure_projection_normal_map": {
            int(key): np.asarray(value, dtype=float).copy()
            for key, value in getattr(state, "pressure_projection_normal_map", {}).items()
        },
...
def _restore_dynamic_state(state: VolumetricPitoisState, snapshot: dict[str, object]) -> None:
    ...
    state.pressure_projection_scalar = float(snapshot["pressure_projection_scalar"])
    state.pressure_projection_area_map = dict(snapshot["pressure_projection_area_map"])
    state.pressure_projection_normal_map = {
        int(key): np.asarray(value, dtype=float).copy()
        for key, value in snapshot["pressure_projection_normal_map"].items()
    }
...
def _prepare_state(config: VolumetricPitoisConfig) -> VolumetricPitoisState:
    ...
    if imported_target_msh_volume_m3 is not None:
        ...
        state.target_volume_m3 = float(_mesh_volume_m3(state.HC))
