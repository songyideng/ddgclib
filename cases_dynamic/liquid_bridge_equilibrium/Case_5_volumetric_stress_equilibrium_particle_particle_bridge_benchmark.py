...
def _subsample_outer_ring_coords(
    outer_ring_coords,
    axial_ring_stride: int,
):
    axial_ring_stride = max(1, int(axial_ring_stride))
    if axial_ring_stride == 1 or len(outer_ring_coords) <= 2:
        return outer_ring_coords

    keep = [0]
    keep.extend(range(axial_ring_stride, len(outer_ring_coords) - 1, axial_ring_stride))
    if keep[-1] != len(outer_ring_coords) - 1:
        keep.append(len(outer_ring_coords) - 1)
    return [outer_ring_coords[idx] for idx in keep]


def _insert_neck_outer_ring_coords(
    outer_ring_coords,
    neck_extra_axial_layers: int,
):
    """Insert additional axial rings in the two cells adjacent to the neck."""
    extra = max(0, int(neck_extra_axial_layers))
    if extra <= 0 or len(outer_ring_coords) <= 2:
        return outer_ring_coords

    z_vals = np.asarray([float(ring[0][2]) for ring in outer_ring_coords], dtype=float)
    ...
    fractions = (z_vals - z_min) / span
    center_idx = int(np.argmin(np.abs(fractions - 0.5)))
    if center_idx <= 0 or center_idx >= len(outer_ring_coords) - 1:
        return outer_ring_coords
    ...
    for q in range(1, n_lower + 1):
        extra_fractions.append(f_lower + (f_center - f_lower) * float(q) / float(n_lower + 1))
    for q in range(1, n_upper + 1):
        extra_fractions.append(f_center + (f_upper - f_center) * float(q) / float(n_upper + 1))

    first_ring = outer_ring_coords[0]
    angles = [math.atan2(float(coord[1]), float(coord[0])) for coord in first_ring]
    existing = {round(float(f), 12) for f in fractions}
    all_fractions = sorted(existing | {round(float(f), 12) for f in extra_fractions})

    refined = []
    for frac in all_fractions:
        z = z_min + float(frac) * span
        refined.append([_catenoid_xyz(theta, z) for theta in angles])
    return refined


def _insert_contact_outer_ring_coords(
    outer_ring_coords,
    contact_extra_axial_layers: int,
):
    """Insert additional axial rings in the cells adjacent to both contact lines."""
    extra_total = max(0, int(contact_extra_axial_layers))
    if extra_total <= 0 or len(outer_ring_coords) <= 2:
        return outer_ring_coords
    ...
    for q in range(1, lower_extra + 1):
        extra_fractions.append(f0 + (f1 - f0) * float(q) / float(lower_extra + 1))

    fm1 = float(fractions[-2])
    fN = float(fractions[-1])
    for q in range(1, upper_extra + 1):
        extra_fractions.append(fm1 + (fN - fm1) * float(q) / float(upper_extra + 1))
...
def _structured_surface_template_from_ring_coords(outer_ring_coords):
    if not outer_ring_coords:
        return [], [], set()

    n_layers = len(outer_ring_coords)
    n_ring = len(outer_ring_coords[0])
    ...
    for k in range(n_layers - 1):
        for i in range(n_ring):
            ...
            pa = np.asarray(outer_ring_coords[k][i], dtype=float)
            pb = np.asarray(outer_ring_coords[k][(i + 1) % n_ring], dtype=float)
            pc = np.asarray(outer_ring_coords[k + 1][(i + 1) % n_ring], dtype=float)
            pd = np.asarray(outer_ring_coords[k + 1][i], dtype=float)
            if float(np.linalg.norm(pa - pc)) <= float(np.linalg.norm(pb - pd)):
    ...
    return outer_ring_coords, sorted(edges), boundary_indices
...
def _build_structured_volumetric_catenoid(
    ...
    rings_coords = _subsample_outer_ring_coords(rings_coords, axial_ring_stride_override)
    rings_coords = _insert_contact_outer_ring_coords(rings_coords, contact_extra_axial_layers_override)
    rings_coords = _insert_neck_outer_ring_coords(rings_coords, neck_extra_axial_layers_override)
    rings_coords, surface_edges, surface_boundary_indices = _structured_surface_template_from_ring_coords(rings_coords)
    ...
    radial_ring_factors = (
        ...
        else _radial_ring_factors_for_n(len(rings_coords[0]))
    )
