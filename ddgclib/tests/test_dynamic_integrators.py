...
class TestRetopologyOverride:
    def test_can_disable_default_retopology(self, mesh_1d, monkeypatch):
        """retopologize_fn=False should skip the built-in retopology path."""
        from ddgclib.dynamic_integrators import euler_velocity_only
        import ddgclib.dynamic_integrators._integrators_dynamic as dyn

        HC, bV = mesh_1d

        def fail_retopo(*args, **kwargs):
            raise AssertionError("default retopology should have been skipped")

        monkeypatch.setattr(dyn, "_retopologize", fail_retopo)
        ...
        assert abs(t - 0.02) < 1e-12

    def test_custom_retopology_hook_is_called(self, mesh_1d):
        """A custom retopology callable should run once per step."""
        from ddgclib.dynamic_integrators import euler_velocity_only

        HC, bV = mesh_1d
        expected = (sum(1 for _ in HC.V), len(bV), 1)
        calls = []

        def my_retopo(hc, boundary_verts, dim, **kwargs):
            calls.append((sum(1 for _ in hc.V), len(boundary_verts), dim))
