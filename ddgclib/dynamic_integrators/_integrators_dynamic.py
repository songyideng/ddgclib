...
def _maybe_retopologize(
    HC,
    bV,
    dim,
    *,
    boundary_filter=None,
    retopologize_fn=None,
    retopologize_kwargs=None,
):
    """Run the requested mesh-maintenance routine for a time step."""
    if retopologize_fn is False:
        return
    if retopologize_fn is None:
        _retopologize(HC, bV, dim, boundary_filter=boundary_filter)
        return
    kwargs = dict(retopologize_kwargs or {})
    retopologize_fn(HC, bV, dim, **kwargs)
...
def euler(HC, bV, dudt_fn, dt, n_steps, dim=3, callback=None, bc_set=None,
          ...
          boundary_filter=None, retopologize_fn=None,
          retopologize_kwargs=None, **dudt_kwargs):
    ...
    retopologize_fn : callable, False, or None
        Optional override for mesh maintenance. ``None`` uses the built-in
        volumetric retopology, ``False`` skips retopology, and a callable is
        invoked once per step as ``retopologize_fn(HC, bV, dim, **kwargs)``.
    retopologize_kwargs : dict or None
        Extra keyword arguments forwarded to *retopologize_fn* when callable.
    **dudt_kwargs
    ...
    for step in range(n_steps):
        _maybe_retopologize(
            ...
            retopologize_fn=retopologize_fn,
            retopologize_kwargs=retopologize_kwargs,
        )
...
def symplectic_euler(HC, bV, dudt_fn, dt, n_steps, dim=3, callback=None,
                     ...
                     retopologize_fn=None, retopologize_kwargs=None,
                     **dudt_kwargs):
...
def rk45(HC, bV, dudt_fn, dt, n_steps, dim=3, callback=None, bc_set=None,
    ...
    retopologize_fn : callable, False, or None
        Optional override for mesh maintenance. ``None`` uses the built-in
        volumetric retopology, ``False`` skips retopology, and a callable is
        invoked once per macro step as ``retopologize_fn(HC, bV, dim, **kwargs)``.
    retopologize_kwargs : dict or None
        Extra keyword arguments forwarded to *retopologize_fn* when callable.
    **dudt_kwargs
