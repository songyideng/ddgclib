...
def hndA_i_interface(v, interface_set, n_i=None):
    """Mean curvature normal restricted to an interface sub-mesh.

    This mirrors :func:`hndA_i`, but only counts neighbors and triangles whose
    vertices are all part of ``interface_set``.  The helper is used by the
    multiphase stress operators to recover the sharp-interface surface
    tension force on interface vertices.
    """
    del n_i  # kept for API compatibility
...
def _pad3(x_a):
    """Pad a coordinate vector to 3D for ``np.cross`` compatibility."""
    if len(x_a) >= 3:
        return x_a[:3]
    out = np.zeros(3)
    out[:len(x_a)] = x_a
